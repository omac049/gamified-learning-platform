...
export class EnhancedPlayer extends Physics.Arcade.Sprite {
  ...
  createHealthBar() {
    ...
    this.healthBarBg = this.scene.add
      .rectangle(this.x, this.y - 50, 60, 8, 0x8b0000)
      .setDepth(this.depth + 1)
      .setAlpha(0.8);

    this.healthBarFill = this.scene.add
      .rectangle(this.x - 30, this.y - 50, 60, 6, 0x32cd32)
      .setOrigin(0, 0.5)
      .setDepth(this.depth + 2);

    // Health bar border
    this.healthBarBorder = this.scene.add
      .rectangle(this.x, this.y - 50, 62, 10, 0x000000, 0)
  ...
  createEnergyBar() {
    ...
    this.energyBarBg = this.scene.add
      .rectangle(this.x, this.y - 40, 60, 6, 0x000080)
      .setDepth(this.depth + 1)
      .setAlpha(0.8);

    this.energyBarFill = this.scene.add
      .rectangle(this.x - 30, this.y - 40, 60, 4, 0x00ffff)
      .setOrigin(0, 0.5)
      .setDepth(this.depth + 2);

    this.energyBarBorder = this.scene.add
      .rectangle(this.x, this.y - 40, 62, 8, 0x000000, 0)
  ...
  createShieldEffect() {
    ...
    this.shieldEffect = this.scene.add
      .circle(this.x, this.y, 45, 0x00ffff, 0)
  ...
  createMuzzleFlash() {
    ...
    const flash = this.scene.add
      .circle(this.x + 20, this.y, 15, 0xffffff, 0.8)
      .setBlendMode('ADD')
      .setDepth(this.depth + 1);
    ...
    // Muzzle particles
    const muzzleParticles = this.scene.add.particles(
      this.x + 20,
      this.y,
      'player-particle',
      {
  ...
  createClickEffect() {
    ...
    const clickParticles = this.scene.add.particles(
      this.x,
      this.y,
      'player-particle',
      {
  ...
  createDamageEffect() {
    ...
    // Red flash
    const damageFlash = this.scene.add
      .rectangle(
    ...
    // Damage particles
    const damageParticles = this.scene.add.particles(
      this.x,
      this.y,
      'player-particle',
      {
  ...
  createHealingEffect() {
    // Healing particles
    const healParticles = this.scene.add.particles(
      this.x,
      this.y - 20,
      'player-particle',
      {
    ...
    // Green glow effect
    const healGlow = this.scene.add
      .circle(this.x, this.y, 50, 0x00ff00, 0.3)
      .setBlendMode('ADD')
      .setDepth(this.depth - 1);
  ...
  createPowerUpEffect(powerUpType) {
    ...
    // Power-up aura
    const powerUpAura = this.scene.add
      .circle(this.x, this.y, 60, color, 0.2)
      .setBlendMode('ADD')
      .setDepth(this.depth - 1);
  ...
  levelUp(newLevel) {
    ...
    const levelUpParticles = this.scene.add.particles(
      this.x,
      this.y,
      'player-particle',
      {
    ...
    const levelText = this.scene.add
      .text(this.x, this.y - 80, `LEVEL ${newLevel}!`, {
        fontSize: '32px',
  ...
  start() {
    ...
    // Entrance particles
    const entranceParticles = this.scene.add.particles(
      this.x,
      this.y,
      'player-particle',
      {
