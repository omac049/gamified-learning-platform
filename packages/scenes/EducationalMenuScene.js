...
export class EducationalMenuScene extends Scene {
  ...
  calculateLayout() {
    ...
    if (screenWidth < 1024) {
      ...
      this.layout.contentWidth =
        screenWidth - this.layout.sidebarWidth - this.layout.padding * 3;
    }
  ...
  create() {
    ...
    if (window.cleanupCorruptedInteractives) {
      ...
      if (cleanedCount > 0) {
        console.log(
          `EducationalMenuScene: Cleaned up ${cleanedCount} corrupted objects before scene creation`
        );
      }
    }
  ...
  createModernBackground() {
    ...
    bgGradient.fillGradientStyle(
      this.uiColors.bgPrimary,
      this.uiColors.bgPrimary,
      this.uiColors.bgSecondary,
      this.uiColors.bgSecondary,
      1
    );
  ...
  createModernHeader() {
    ...
    const title = this.add
      .text(this.scale.width / 2, 40, 'CYBER ACADEMY', {
        fontSize: '42px',
    ...
    const subtitle = this.add
      .text(this.scale.width / 2, 70, 'ROBOT COMMAND CENTER', {
        fontSize: '18px',
        fontFamily: 'Courier, monospace',
        fill: '#ffd700',
        stroke: '#000000',
  ...
  createPlayerStatusBar() {
    ...
    const playerText = this.add
      .text(this.layout.padding + 15, 102, playerInfo, {
        fontSize: '14px',
        fontFamily: 'Courier, monospace',
        fill: '#ffffff',
        fontStyle: 'bold',
  ...
  createModernSidebar() {
    ...
    sidebarBg.fillRoundedRect(
      sidebarX,
      this.layout.headerHeight,
      this.layout.sidebarWidth,
      this.scale.height - this.layout.headerHeight,
      0
    );
    sidebarBg.lineStyle(1, this.uiColors.neonBlue, 0.3);
    sidebarBg.strokeRoundedRect(
      sidebarX,
      this.layout.headerHeight,
      this.layout.sidebarWidth,
      this.scale.height - this.layout.headerHeight,
  ...
  createCharacterCard(x, y) {
    ...
    try {
      ...
      if (this.textures.exists(textureKey)) {
        robotSprite = this.add
          .sprite(x + 120, y + 60, textureKey)
          .setScale(2.5)
          .setOrigin(0.5);
      } else {
      ...
      console.warn(
        'EducationalMenuScene: Could not load robot sprite, using fallback:',
        error
      );
      // Fallback: Create a styled robot representation using graphics
      robotSprite = this.createRobotGraphic(x + 120, y + 60, charType);
    ...
    const nameText = this.add
      .text(x + 120, y + 110, character.name, {
        fontSize: '16px',
        fontFamily: 'Courier, monospace',
        fill: '#ffffff',
        fontStyle: 'bold',
    ...
    const typeText = this.add
      .text(x + 120, y + 130, charType.name, {
        fontSize: '12px',
        fontFamily: 'Courier, monospace',
    ...
    const levelText = this.add
      .text(x + 120, y + 150, `Level ${progress.characterLevel}`, {
        fontSize: '14px',
        fontFamily: 'Courier, monospace',
        fill: '#ffd700',
        fontStyle: 'bold',
  ...
  addUpgradeIndicators(centerX, centerY, charType) {
    ...
    upgradePositions.forEach(pos => {
      const hasUpgrade =
        equippedItems[pos.type] !== null &&
        equippedItems[pos.type] !== undefined;

      if (hasUpgrade) {
        // Create upgrade indicator
        const upgradeBg = this.add.circle(
          pos.x,
          pos.y,
          12,
          charType.accentColor,
          0.8
        );
        upgradeBg.setStrokeStyle(2, charType.accentColor);
        this.sidebarContainer.add(upgradeBg);

        const upgradeIcon = this.add
          .text(pos.x, pos.y, pos.icon, {
            fontSize: '16px',
          })
          .setOrigin(0.5);
    ...
    const actions = [
      {
        icon: '🔧',
        text: 'Robot Shop',
        color: this.uiColors.neonGreen,
        action: () => this.openShop(),
      },
      {
        icon: '💰',
        text: 'Daily Coins',
        color: this.uiColors.neonYellow,
        action: () => this.claimDailyReward(),
      },
      {
        icon: '📊',
        text: 'Progress',
        color: this.uiColors.neonPurple,
        action: () => this.showProgressDashboard(),
      },
    ];
    ...
    actions.forEach((action, index) => {
      ...
      if (this.safeSetInteractive(buttonBg, null, { useHandCursor: true })) {
        ...
        const buttonText = this.add
          .text(x + 20, buttonY + 20, `${action.icon} ${action.text}`, {
            fontSize: '14px',
            fontFamily: 'Courier, monospace',
            fill: '#ffffff',
            fontStyle: 'bold',
  ...
  createProgressSummary(x, y) {
    ...
    const titleText = this.add
      .text(x + 120, y + 20, 'PROGRESS SUMMARY', {
        fontSize: '12px',
        fontFamily: 'Courier, monospace',
  ...
  createMainContent() {
    ...
    contentBg.fillRoundedRect(
      contentX,
      contentY,
      contentWidth,
      this.scale.height - contentY - this.layout.padding,
      16
    );
  ...
  createWeekCards(x, y, maxWidth) {
    const weeks = [
      {
        number: 1,
        title: 'Math Combat Arena',
        theme: 'Epic Robot Battles',
        color: this.uiColors.success,
        icon: '⚔️',
        difficulty: 'Easy',
        status: '✅ Full Combat',
      },
      {
        number: 2,
        title: 'Reading Combat Arena',
        theme: 'Cyber Battle Challenges',
        color: this.uiColors.warning,
        icon: '📚',
        difficulty: 'Easy',
        status: '✅ Full Combat',
      },
      {
        number: 3,
        title: 'Science Combat Lab',
        theme: 'Experimental Battles',
        color: this.uiColors.neonPurple,
        icon: '🔬',
        difficulty: 'Medium',
        status: '🔄 Combat Ready',
      ...
      {
        number: 4,
        title: 'History Battle Arena',
        theme: 'Time War Missions',
        color: this.uiColors.error,
        icon: '🚀',
        difficulty: 'Medium',
        status: '⏳ Combat Planned',
      },
      {
        number: 5,
        title: 'Multi-Subject Combat',
        theme: 'Ultimate Challenge',
        color: this.uiColors.info,
        icon: '👑',
        difficulty: 'Hard',
        status: '⏳ Boss Battles',
      },
      {
        number: 6,
        title: 'Final Robot War',
        theme: 'Legendary Battle',
        color: this.uiColors.accent,
        icon: '🎓',
        difficulty: 'Expert',
        status: '⏳ Epic Finale',
      },
    ...
    const cardWidth =
      (maxWidth - this.layout.cardSpacing * (cardsPerRow - 1)) / cardsPerRow;
  ...
  createWeekCard(week, x, y, width, height) {
    ...
    const isCompleted = this.progressTracker.progress.weeksCompleted.includes(
      week.number
    );
    ...
    cardBg.lineStyle(
      2,
      isUnlocked ? week.color : this.uiColors.textMuted,
      isUnlocked ? 0.6 : 0.3
    );
    ...
    if (week.status) {
      ...
      const statusDisplayText = this.add
        .text(110 + (width - 130) / 2, 110, week.status, {
          fontSize: '9px',
          fontFamily: 'Courier, monospace',
          fill: '#ffffff',
          fontStyle: 'bold',
    ...
    if (isUnlocked) {
      console.log(
        `Setting up interactive for Week ${week.number}: ${week.title}`
      );
  ...
  getDifficultyColor(difficulty) {
    switch (difficulty) {
      case 'Easy':
        return this.uiColors.success;
      case 'Medium':
        return this.uiColors.warning;
      case 'Hard':
        return this.uiColors.error;
      case 'Expert':
        return this.uiColors.neonPurple;
      default:
        return this.uiColors.info;
    }
  }
  ...
  createFloatingActions() {
    ...
    if (hasReward) {
      const fabBg = this.add.circle(
        this.scale.width - 80,
        this.scale.height - 80,
        30,
        this.uiColors.neonYellow,
        0.9
      );
      ...
      if (this.safeSetInteractive(fabBg, null, { useHandCursor: true })) {
        ...
        const fabIcon = this.add
          .text(this.scale.width - 80, this.scale.height - 80, '💰', {
            fontSize: '24px',
          })
          .setOrigin(0.5);
    ...
    if (this.safeSetInteractive(helpBg, null, { useHandCursor: true })) {
      ...
      const helpIcon = this.add
        .text(this.scale.width - 80, this.scale.height - 140, '?', {
          fontSize: '20px',
          fontFamily: 'Courier, monospace',
          fill: '#ffffff',
          fontStyle: 'bold',
  ...
  showHelpModal() {
    try {
      ...
      if (!this.add || !this.scale || !this.uiColors) {
        console.warn(
          'EducationalMenuScene: Cannot show help modal - scene not ready'
        );
        return;
      }
    ...
    Press ESC or click CLOSE to return to the academy!`;
        ...
        const helpText = this.add
          .text(this.scale.width / 2, this.scale.height / 2 - 50, helpContent, {
            fontSize: '12px',
            fontFamily: 'Courier, monospace',
            fill: '#ffffff',
            align: 'left',
  ...
  startWeek(weekNumber) {
    ...
    const transitionOverlay = this.add
    ...
    const loadingSpinner = this.add
      .text(this.scale.width / 2, this.scale.height / 2 + 50, '⚙️', {
        fontSize: '32px',
      })
      .setOrigin(0.5)
      .setDepth(1001)
  ...
  performSceneTransition(weekNumber) {
    console.log(
      `EducationalMenuScene: Performing transition to Week ${weekNumber}`
    );
    ...
    this.children.list.forEach(child => {
      if (child.input) {
        try {
          ...
          console.warn(
            'EducationalMenuScene: Error removing listeners during cleanup:',
            error.message
          );
        }
      }
  ...
  showComingSoonMessage(weekTitle) {
    ...
    const comingSoonTitle = this.add
      .text(
        this.scale.width / 2,
    ...
    if (this.safeSetInteractive(closeBtn, null, { useHandCursor: true })) {
      ...
      const closeBtnText = this.add
        .text(this.scale.width / 2, this.scale.height / 2 + 100, 'OK', {
          fontSize: '16px',
          fontFamily: 'Courier, monospace',
          fill: '#ffffff',
          fontStyle: 'bold',
      ...
      console.warn(
        'EducationalMenuScene: Failed to set interactive on close button in coming soon modal'
      );
    }
  ...
  showErrorMessage(message) {
    ...
    const errorTitle = this.add
      .text(this.scale.width / 2, this.scale.height / 2 - 50, '⚠️ ERROR', {
        fontSize: '24px',
        fontFamily: 'Courier, monospace',
        fill: '#ef4444',
        fontStyle: 'bold',
    ...
    const errorText = this.add
      .text(this.scale.width / 2, this.scale.height / 2 - 10, message, {
        fontSize: '16px',
        fontFamily: 'Courier, monospace',
        fill: '#ffffff',
        align: 'center',
    ...
    if (this.safeSetInteractive(closeBtn, null, { useHandCursor: true })) {
      ...
      const closeBtnText = this.add
        .text(this.scale.width / 2, this.scale.height / 2 + 60, 'CLOSE', {
          fontSize: '16px',
          fontFamily: 'Courier, monospace',
          fill: '#ffffff',
          fontStyle: 'bold',
      ...
      console.warn(
        'EducationalMenuScene: Failed to set interactive on close button in error modal'
      );
    }
  }
  ...
  showProgressDashboard() {
    ...
    modalPanel.lineStyle(
      2,
      charType ? charType.accentColor : this.uiColors.neonBlue,
      0.8
    );
    modalPanel.strokeRoundedRect(
      this.scale.width / 2 - 350,
    ...
    const dashboardTitle = this.add
      .text(this.scale.width / 2, yPos, '📊 PROGRESS DASHBOARD', {
        fontSize: '28px',
        fontFamily: 'Courier, monospace',
        fill: '#ffffff',
        fontStyle: 'bold',
    ...
    // Subject accuracies
    const subjectTitle = this.add
      .text(this.scale.width / 2, yPos, 'SUBJECT PERFORMANCE', {
        fontSize: '18px',
        fontFamily: 'Courier, monospace',
        fill: '#fbbf24',
        fontStyle: 'bold',
    ...
    subjects.forEach((subject, index) => {
      ...
      const accuracyColor =
        accuracy >= 80
          ...
          : accuracy >= 60
            ? this.uiColors.warning
            : this.uiColors.error;
      accuracyBar.fillStyle(accuracyColor);
      accuracyBar.fillRoundedRect(
        this.scale.width / 2 - 100,
        subjectY,
        200 * (accuracy / 100),
        16,
      ...
      const accuracyText = this.add
        .text(this.scale.width / 2 + 120, subjectY + 8, `${accuracy}%`, {
          fontSize: '12px',
          fontFamily: 'Courier, monospace',
          fill: '#ffffff',
          fontStyle: 'bold',
    ...
    if (this.safeSetInteractive(closeBtn, null, { useHandCursor: true })) {
      const closeBtnText = this.add
        .text(this.scale.width / 2, this.scale.height / 2 + 220, 'CLOSE', {
          fontSize: '16px',
          fontFamily: 'Courier, monospace',
          fill: '#ffffff',
          fontStyle: 'bold',
  ...
  safeSetInteractive(gameObject, hitArea, options = {}) {
    try {
      ...
      if (!gameObject || !gameObject.scene || gameObject.scene !== this) {
        console.warn(
          'EducationalMenuScene: Attempted to set interactive on invalid or destroyed object'
        );
        return false;
      }
      ...
      if (gameObject.input) {
        ...
        if (
          ...
          console.warn(
            'EducationalMenuScene: Detected corrupted hitAreaCallback, clearing it'
          );
      ...
      console.error(
        'EducationalMenuScene: Error in safeSetInteractive:',
        error
      );
      ...
      try {
        ...
        console.warn(
          'EducationalMenuScene: Could not clean up after setInteractive error:',
  ...
  applyInteractiveState(gameObject, hitArea, options) {
    try {
      ...
      if (!gameObject || !gameObject.scene || gameObject.scene !== this) {
        console.warn(
          'EducationalMenuScene: Game object became invalid during interactive setup'
        );
        return false;
      }
      ...
      if (hitArea) {
        ...
        if (hitArea instanceof Phaser.Geom.Rectangle) {
          ...
          console.warn(
            'EducationalMenuScene: Unknown hit area type, using auto-detection'
          );
      ...
      if (gameObject.input) {
        if (
          ...
          console.warn(
            'EducationalMenuScene: Interactive setup resulted in corrupted hitAreaCallback, removing interactive'
          );
          gameObject.removeInteractive();
          return false;
        }
        ...
        if (gameObject.type === 'Graphics' && gameObject.input.hitArea) {
          ...
          if (
            ...
            console.warn(
              'EducationalMenuScene: Graphics object has invalid hit area, removing interactive'
            );
            gameObject.removeInteractive();
            return false;
          }
      ...
      console.error(
        'EducationalMenuScene: Error applying interactive state:',
        error
      );
      // Clean up on error
      try {
        ...
        console.warn(
          'EducationalMenuScene: Could not clean up after interactive error:',
          cleanupError.message
        );
      }
      return false;
  ...
  createRobotGraphic(x, y, charType) {
    try {
      ...
      if (!this.add || !charType) {
        console.warn(
          'EducationalMenuScene: Cannot create robot graphic - scene not ready or invalid charType'
        );
        return this.createSimpleRobotFallback(x, y, charType);
      }

      console.log(
        `Creating ultra-detailed robot graphic for ${charType.name} at (${x}, ${y})`
      );
      ...
      try {
        ...
        console.error(
          'EducationalMenuScene: Error creating robot components:',
          componentError
        );
      ...
      console.error(
        'EducationalMenuScene: Error creating robot graphic:',
        error
      );
      return this.createSimpleRobotFallback(x, y, charType);
    }
  ...
  createSimpleRobotFallback(x, y, charType) {
    try {
      if (!this.add) {
        console.error(
          'EducationalMenuScene: Cannot create fallback robot - scene not ready'
        );
        return null;
      }
      ...
      console.error(
        'EducationalMenuScene: Error creating fallback robot:',
        error
      );
      return null;
    }
  ...
  createRobotTorso(container, specs) {
    ...
    if (specs.features?.bulky) {
      ...
      // Angular torso shape
      const torsoPath = [-15, -10, 15, -10, 20, 0, 15, 25, -15, 25, -20, 0];
  ...
  createRobotHead(container, specs) {
    ...
    if (specs.features?.bulky) {
      ...
      // Angular head shape
      const headPath = [-10, -35, 10, -35, 12, -25, 8, -15, -8, -15, -12, -25];
      head.fillPoints(headPath, true);

      // Holographic visor
      head.fillStyle(specs.glowColor, 0.7);
      head.fillPoints(
        [-8, -32, 8, -32, 10, -26, 6, -20, -6, -20, -10, -26],
        true
      );
  ...
  createSteamVents(container, specs) {
    ...
    ventPositions.forEach((pos, index) => {
      ...
      this.time.addEvent({
        ...
        callback: () =>
          this.createSteamParticle(container, pos.x, pos.y, specs),
        loop: true,
      });
    });
  }
