...
import { SaveManager } from '../utils/managers/index.js';
...
export class IntroScene extends Scene {
  ...
  createRetroBackground() {
    ...
    bg.fillGradientStyle(
      this.retroColors.darkPurple,
      this.retroColors.darkPurple,
      this.retroColors.darkBlue,
      this.retroColors.darkBlue,
      1
    );
  ...
  createRetroTitle() {
    ...
    const glitchBg = this.add.rectangle(
      this.scale.width / 2,
      titleY,
      800,
      80,
    ...
    const mainTitle = this.add
      .text(this.scale.width / 2, titleY - 15, 'CYBER BATTLE', {
        fontSize: '42px',
    ...
    const subtitle = this.add
      .text(this.scale.width / 2, titleY + 15, 'ACADEMY 2085', {
        fontSize: '24px',
        fontFamily: 'Courier, monospace',
        fill: '#ffff00',
        stroke: '#ff8000',
  ...
  createRetroUI() {
    ...
    const statusPanel = this.add
      .rectangle(statusX, statusY, 260, 100, 0x000000, 0.7)
      .setStrokeStyle(2, this.retroColors.neonGreen);

    this.add
      .text(statusX, statusY - 35, 'BATTLE SYSTEMS', {
        fontSize: '12px',
        fontFamily: 'Courier, monospace',
        fill: '#00ff00',
        fontStyle: 'bold',
    ...
    this.add
      .text(statusX, statusY - 15, 'NEURAL LINK: ACTIVE', {
        fontSize: '10px',
        fontFamily: 'Courier, monospace',
        fill: '#00ffff',
      })
    ...
    this.add
      .text(statusX, statusY, 'COMBAT PROTOCOLS: READY', {
        fontSize: '10px',
        fontFamily: 'Courier, monospace',
        fill: '#00ffff',
      })
    ...
    this.add
      .text(statusX, statusY + 15, 'FIGHTER SELECTION: PENDING', {
        fontSize: '10px',
        fontFamily: 'Courier, monospace',
        fill: '#ffff00',
      })
    ...
    const timerPanel = this.add
      .rectangle(timerX, timerY, 260, 100, 0x000000, 0.7)
      .setStrokeStyle(2, this.retroColors.neonPink);

    this.add
      .text(timerX, timerY - 35, 'BATTLE TIMER', {
        fontSize: '12px',
        fontFamily: 'Courier, monospace',
        fill: '#ff00ff',
        fontStyle: 'bold',
    ...
    const timeDisplay = this.add
      .text(timerX, timerY - 10, '2085.12.25', {
        fontSize: '14px',
        fontFamily: 'Courier, monospace',
        fill: '#00ffff',
        fontStyle: 'bold',
    ...
    const clockDisplay = this.add
      .text(timerX, timerY + 10, '00:00:00', {
        fontSize: '12px',
        fontFamily: 'Courier, monospace',
        fill: '#ffff00',
      })
    ...
    this.add
      .text(timerX, timerY + 30, 'CYBER TIME ACTIVE', {
        fontSize: '9px',
        fontFamily: 'Courier, monospace',
        fill: '#cccccc',
      })
  ...
  createRetroDecorations() {
    ...
    const sideLines = [
      ...
      {
        x1: this.scale.width - 30,
        y1: this.scale.height / 2,
        x2: this.scale.width,
        y2: this.scale.height / 2,
      },
    ];
  ...
  createRetroNavigation() {
    ...
    this.add
      .text(this.scale.width / 2, navY - 40, 'BATTLE CONTROL', {
        fontSize: '16px',
        fontFamily: 'Courier, monospace',
        fill: '#ff00ff',
        fontStyle: 'bold',
    ...
    const instructionsGlow = this.add.rectangle(
      startX,
      navY,
      210,
      55,
      this.retroColors.neonBlue,
      0.1
    );

    // Start button with retro arcade styling
    const startButton = this.add
      .rectangle(startX + buttonSpacing, navY, 200, 45, 0x000000, 0.8)
      .setStrokeStyle(3, this.retroColors.neonPink)
      .setInteractive({ useHandCursor: true });

    const startText = this.add
      .text(startX + buttonSpacing, navY, '>> ENTER BATTLE <<', {
        fontSize: '14px',
        fontFamily: 'Courier, monospace',
        fill: '#ff00ff',
        fontStyle: 'bold',
    ...
    this.add
      .text(
        this.scale.width / 2,
        navY + 35,
        'CLICK TO PLAY • HAVE FUN LEARNING',
        {
          fontSize: '9px',
          fontFamily: 'Courier, monospace',
          fill: '#cccccc',
  ...
  showRetroEntranceAnimation() {
    ...
    const scanline = this.add
      .rectangle(0, 0, this.scale.width, 4, this.retroColors.neonBlue)
      .setOrigin(0, 0);
    ...
    const elements = this.children.list.filter(
      child => child !== scanline && child.alpha !== undefined
    );
  ...
  showReturningPlayerWelcome(saveInfo) {
    ...
    const welcomePanel = this.add
      .rectangle(this.scale.width / 2, welcomeY, 400, 60, 0x000000, 0.8)
      .setStrokeStyle(2, this.retroColors.neonGreen)
      .setDepth(1000); // High Z-order to appear above other elements

    const welcomeTitle = this.add
      .text(
        this.scale.width / 2,
        welcomeY - 15,
    ...
    const agentText = this.add
      .text(
        this.scale.width / 2,
        welcomeY + 5,
        `FIGHTER: ${saveInfo.character.name.toUpperCase()}`,
        {
          fontSize: '12px',
          fontFamily: 'Courier, monospace',
          fill: '#00ffff',
  ...
  showNewPlayerWelcome() {
    ...
    const introPanel = this.add
      .rectangle(this.scale.width / 2, introY, 500, 60, 0x000000, 0.8)
    ...
    const introSubtext = this.add
      .text(this.scale.width / 2, introY + 5, 'SELECT YOUR FIGHTER TO BEGIN', {
        fontSize: '12px',
        fontFamily: 'Courier, monospace',
        fill: '#ffffff',
        stroke: '#000000',
  ...
  startCyberMission() {
    ...
    this.cameras.main.once('camerafadeoutcomplete', () => {
      if (hasExistingCharacter) {
        console.log(
          'IntroScene: Returning player detected, going to main menu'
        );
        this.scene.start('EducationalMenuScene');
      } else {
        console.log(
          'IntroScene: New player detected, going to character creation'
        );
  ...
  showGameplayInstructions() {
    ...
    this.add
      .text(this.scale.width / 2, this.scale.height / 2 - 195, 'HOW TO PLAY', {
        fontSize: '18px',
        fontFamily: 'Courier, monospace',
        fill: '#00ffff',
        fontStyle: 'bold',
...
SAVING: Don't worry - we save your games!`;
    ...
    const closeText = this.add
      .text(this.scale.width / 2, this.scale.height / 2 + 200, '>> CLOSE <<', {
        fontSize: '14px',
        fontFamily: 'Courier, monospace',
        fill: '#ff00ff',
        fontStyle: 'bold',
