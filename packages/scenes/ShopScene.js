...
export class ShopScene extends Scene {
  ...
  create() {
    ...
    // Shop title
    this.add
      .text(this.scale.width / 2, 50, '🔧 ROBOT UPGRADE SHOP 🔧', {
        fontSize: '32px',
    ...
    return {
      armor: [
        // ARIA Cyber Armor Upgrades
        {
          ...
          description:
            'Basic cyber defense upgrade (+5% defense, +3% analysis)',
          price: 100,
          icon: '🛡️',
          color: 0x3b82f6,
          permanent: true,
        ...
        },
        {
          ...
          description:
            'Enhanced processing power (+8% reading, +5% efficiency)',
          price: 150,
          icon: '⚡',
          color: 0x00ffff,
          permanent: true,
        ...
        },
        {
          ...
          description:
            'Advanced energy shields (+15% defense, +12% technology)',
          price: 220,
          icon: '🛡️',
          color: 0x10b981,
          permanent: true,
        ...
        },
        {
          ...
          description:
            'Quantum processing upgrade (+12% science, +8% innovation)',
          price: 160,
          icon: '⚡',
          color: 0xfbbf24,
          permanent: true,
  ...
  createCategoryTabs() {
    ...
    categories.forEach((category, index) => {
      ...
      const tab = this.add
        .rectangle(x, y, tabWidth - 10, 50, 0x374151)
        .setStrokeStyle(2, 0x6b7280)
        .setInteractive({ useHandCursor: true });

      const tabText = this.add
        .text(x, y, `${category.icon} ${category.name}`, {
  ...
  createNavigationButtons() {
    ...
    const backBtn = this.add
      .rectangle(50, this.scale.height - 40, 120, 35, 0x6366f1)
      .setStrokeStyle(2, 0xffffff)
      .setInteractive({ useHandCursor: true });

    this.add
      .text(50, this.scale.height - 40, '← Back', {
        fontSize: '16px',
        fontFamily: 'Arial, sans-serif',
        fill: '#ffffff',
        fontStyle: 'bold',
    ...
    const inventoryBtn = this.add
      .rectangle(200, this.scale.height - 40, 120, 35, 0x10b981)
      .setStrokeStyle(2, 0xffffff)
      .setInteractive({ useHandCursor: true });

    this.add
      .text(200, this.scale.height - 40, '📦 Inventory', {
        fontSize: '14px',
        fontFamily: 'Arial, sans-serif',
        fill: '#ffffff',
        fontStyle: 'bold',
  ...
  createItemCard(item, x, y, width, height) {
    ...
    const currentMechType = this.progressTracker.getCharacterTypeId();
    const isForCurrentMech =
      !item.mechType || item.mechType === currentMechType;
    ...
    const card = this.add
      .rectangle(x, y, width, height, cardColor)
  ...
  showItemDetails(item) {
    ...
    this.detailsPanelContent.add(
      this.add
        .text(panelX, yPos, `${item.icon} ${item.name}`, {
          fontSize: '18px',
          fontFamily: 'Arial, sans-serif',
          fill: '#ffd700',
          fontStyle: 'bold',
    ...
    this.detailsPanelContent.add(
      this.add
        .text(panelX, yPos, item.description, {
          fontSize: '12px',
          fontFamily: 'Arial, sans-serif',
          fill: '#ffffff',
          wordWrap: { width: 180 },
    ...
    this.detailsPanelContent.add(
      this.add
        .text(panelX, yPos, `💰 ${item.price} Coins`, {
          fontSize: '14px',
          fontFamily: 'Arial, sans-serif',
          fill: '#ffd700',
          fontStyle: 'bold',
    ...
    if (item.bonuses) {
      ...
      Object.entries(item.bonuses).forEach(([stat, value]) => {
        this.detailsPanelContent.add(
          this.add
            .text(panelX, yPos, `+${value}% ${stat.toUpperCase()}`, {
              fontSize: '11px',
              fontFamily: 'Arial, sans-serif',
              fill: '#ffffff',
            })
    ...
    if (item.requires) {
      const hasRequirement = this.progressTracker.hasItem(
        item.requires,
        'armor'
      );
      ...
      const requiredItem = this.shopItems.armor.find(
        armor => armor.id === item.requires
      );
      ...
      this.detailsPanelContent.add(
        this.add
          .text(
            panelX,
            yPos,
            hasRequirement ? `✅ ${reqName}` : `❌ ${reqName}`,
            {
              fontSize: '11px',
              fontFamily: 'Arial, sans-serif',
      ...
      this.detailsPanelContent.add(
        this.add
          .text(
            ...
            yPos,
            isCorrectMech
              ? `✅ ${item.mechType.toUpperCase()}`
              : `❌ Need ${item.mechType.toUpperCase()}`,
            {
              fontSize: '11px',
              fontFamily: 'Arial, sans-serif',
    ...
    if (owned && item.consumable) {
      ...
      const useBtn = this.add
        .rectangle(panelX, yPos, 200, 40, 0x10b981)
        .setStrokeStyle(2, 0xffffff)
        .setInteractive({ useHandCursor: true });
      ...
      const purchaseBtn = this.add
        .rectangle(panelX, yPos, 200, 40, btnColor)
        .setStrokeStyle(2, 0xffffff)
      ...
      this.detailsPanelContent.add(
        this.add
          .text(panelX, yPos, canAfford ? 'Purchase' : 'Not Enough Coins', {
            fontSize: '16px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffffff',
            fontStyle: 'bold',
      ...
      this.detailsPanelContent.add(
        this.add
          .text(panelX, yPos, '❌ REQUIREMENTS NOT MET', {
            fontSize: '14px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ff6b6b',
            fontStyle: 'bold',
  ...
  purchaseItem(item) {
    if (
      this.progressTracker.purchaseItem(
        item.id,
  ...
  showInventory() {
    ...
    ['cosmetics', 'tools', 'decorations'].forEach(category => {
      if (inventory[category].length > 0) {
        ...
        inventory[category].forEach(itemId => {
          const item = this.shopItems[category].find(
            item => item.id === itemId
          );
    ...
    const inventoryDisplay = this.add
      .text(this.scale.width / 2, this.scale.height / 2 - 50, inventoryText, {
        fontSize: '14px',
        fontFamily: 'Arial, sans-serif',
        fill: '#ffffff',
        align: 'left',
    ...
    this.add
      .text(this.scale.width / 2, this.scale.height / 2 + 150, 'Close', {
        fontSize: '16px',
        fontFamily: 'Arial, sans-serif',
        fill: '#ffffff',
        fontStyle: 'bold',
  ...
  createCharacterInfoDisplay() {
    ...
    if (!character || !characterType) {
      ...
      this.add.text(
        20,
        80,
        '⚠️ No robot selected. Visit Character Selection first!',
        {
          fontSize: '16px',
          fontFamily: 'Arial, sans-serif',
          fill: '#ff6b6b',
    ...
    const infoBg = this.add
      .rectangle(150, 85, 280, 50, 0x1f2937, 0.9)
      .setStrokeStyle(2, characterType.baseColor);

    // Character icon and name
    this.add
      .text(60, 85, `${characterType.icon} ${character.name}`, {
        fontSize: '18px',
        fontFamily: 'Arial, sans-serif',
        fill: '#ffffff',
        fontStyle: 'bold',
    ...
    this.add
      .text(200, 75, `${characterType.name} - Level ${level}`, {
        fontSize: '14px',
        fontFamily: 'Arial, sans-serif',
