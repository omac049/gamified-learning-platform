...
export class Week1MathScene extends Scene {
  ...
  create() {
    ...
    try {
      ...
      console.log(
        'Week1MathScene: Scene creation complete with Combat System!'
      );
    } catch (error) {
  ...
  initializeCharacterStats() {
    ...
    try {
      ...
      if (character && charType) {
        ...
        console.log(
          'Week1MathScene: Character stats initialized:',
          this.characterStats
        );
      } else {
      ...
      console.error(
        'Week1MathScene: Error initializing character stats:',
        error
      );
    }
  }
  ...
  applyEquipmentBonuses() {
    try {
      ...
      console.log(
        'Week1MathScene: Equipment bonuses applied:',
        this.characterStats
      );
    } catch (error) {
  ...
  createRobotGraphic(x, y, charType) {
    console.log(
      `Creating robot graphic for ${charType.id || 'default'} at (${x}, ${y})`
    );
  ...
  initializeAudio() {
    try {
      ...
      console.warn(
        'Week1MathScene: Audio system failed to initialize:',
        error.message
      );
      this.hasAudio = false;
    }
  ...
  createUI() {
    ...
    this.add.text(healthX - 100, 15, '❤️ Health', {
      ...uiStyle,
      fontSize: '16px',
    });
    ...
    // Math power bar
    this.add.text(healthX - 100, 55, '⚡ Math Power', {
      ...uiStyle,
      fontSize: '16px',
    });
    ...
    this.streakText = this.add.text(healthX - 70, 100, 'Streak: 0', {
      ...uiStyle,
      fontSize: '16px',
    });
  ...
  connectSystemEvents() {
    ...
    if (this.uiManager) {
      ...
      this.events.on('playerHealthChanged', data =>
        this.uiManager.emit('playerHealthChanged', data)
      );
      this.events.on('mathPowerChanged', data =>
        this.uiManager.emit('mathPowerChanged', data)
      );
      this.events.on('streakChanged', data =>
        this.uiManager.emit('streakChanged', data)
      );
    }

    // Connect Math System events
    if (this.mathSystem) {
      this.events.on('enemySpawned', data =>
        this.mathSystem.emit('enemySpawned', data)
      );
      this.events.on('playerHealthLow', data =>
        this.mathSystem.emit('playerHealthLow', data)
      );
      this.events.on('bossEncounter', data =>
        this.mathSystem.emit('bossEncounter', data)
      );
      this.events.on('powerUpRequested', data =>
        this.mathSystem.emit('powerUpRequested', data)
      );
    }
  ...
  onMathAnswerCorrect(data) {
    console.log(
      'Week1MathScene: Correct answer with combat integration!',
      data
    );
    ...
    try {
      ...
      const bonusScore =
        Math.floor(50 * this.characterStats.attackPower) + this.streak * 10;
      ...
      const xpGain = Math.floor(
        10 * (1 + this.characterStats.intelligence / 100)
      );
      ...
      console.log(
        `Combat result: ${damage} damage dealt, ${bonusScore} score gained, ${xpGain} XP`
      );
    } catch (error) {
  ...
  onMathAnswerIncorrect(data) {
    console.log(
      'Week1MathScene: Incorrect answer with combat integration!',
      data
    );
    ...
    try {
      ...
      console.log(
        `Combat result: ${actualPenalty} penalty applied (reduced from ${basePenalty} by ${this.characterStats.defense} defense)`
      );
    } catch (error) {
  ...
  onEnemyDefeated() {
    ...
    const bonusXP = Math.floor(
      50 * (1 + this.characterStats.intelligence / 100)
    );
  ...
  showMathQuizUI() {
    ...
    this.mathQuizBgGlow = this.add.rectangle(
      centerX,
      centerY,
      460,
      360,
    ...
    this.mathQuizQuestion = this.add
      .text(centerX, centerY - 100, this.currentQuestion.question, {
        fontSize: '36px',
        fontFamily: 'Courier, monospace',
        color: '#ffffff',
        stroke: '#00ffff',
    ...
    this.mathQuizInstructions = this.add
      .text(centerX, centerY + 120, 'Press 1, 2, 3, or 4 to answer quickly!', {
        fontSize: '18px',
        fontFamily: 'Courier, monospace',
        color: '#ffffff',
        stroke: '#000000',
    ...
    this.mathQuizTimerBg = this.add.rectangle(
      centerX,
      centerY + 150,
      400,
      10,
    ...
    this.mathQuizTimerBar = this.add.rectangle(
      centerX,
      centerY + 150,
      400,
      10,
  ...
  showAnswerFeedback(selectedIndex, isCorrect) {
    ...
    if (!isCorrect) {
      const correctChoice =
        this.mathQuizChoices[this.currentQuestion.correctIndex];
      if (correctChoice) {
    ...
    const feedback = this.add
      .text(this.scale.width / 2, this.scale.height / 2 + 180, feedbackText, {
        fontSize: '20px',
        fontFamily: 'Courier, monospace',
        color: feedbackColor,
        stroke: '#000000',
  ...
  showTimeoutFeedback() {
    ...
    const feedback = this.add
      .text(this.scale.width / 2, this.scale.height / 2 + 180, timeoutText, {
        fontSize: '18px',
        fontFamily: 'Courier, monospace',
        color: '#ffaa00',
        stroke: '#000000',
  ...
  endGame() {
    ...
    if (this.score > 0) {
      ...
      if (accuracy >= 60 || this.questionsAnswered >= 5) {
        ...
        console.log(
          'Week1MathScene: Week 1 completed! Accuracy:',
          `${accuracy.toFixed(1)}%`
        );
      }
  ...
  showGameOverScreen(accuracy) {
    ...
    const playAgainText = this.add
      .text(centerX - 120, centerY + 130, '🔄 PLAY AGAIN', buttonStyle)
      .setOrigin(0.5);
    ...
    const menuText = this.add
      .text(centerX + 120, centerY + 130, '🏠 MAIN MENU', buttonStyle)
      .setOrigin(0.5);
    ...
    gameOverBg.on('pointerdown', pointer => {
      ...
      if (
        !Phaser.Geom.Rectangle.Contains(playAgainBounds, clickX, clickY) &&
        ...
        try {
          ...
          console.error(
            'Week1MathScene: Error restarting via background click:',
            error
          );
          window.location.reload();
        }
  ...
  goToMainMenu() {
    ...
    for (const sceneName of sceneOptions) {
      try {
        ...
        console.warn(
          `Week1MathScene: Failed to switch to ${sceneName}:`,
          error.message
        );
      }
    }
  ...
  createArena() {
    ...
    this.add
      .text(this.scale.width / 2, 30, '⚔️ MATH COMBAT ARENA ⚔️', {
        fontSize: '24px',
        fontFamily: 'Courier, monospace',
        fill: '#00ffff',
        fontStyle: 'bold',
  ...
  onPlayerEnergyRestore(data) {
    this.playerEnergy = Math.min(
      this.maxPlayerEnergy,
      this.playerEnergy + data.amount
    );
    this.updateUI();
  }
