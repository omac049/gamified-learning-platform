...
export class Week5CrossoverScene extends Scene {
    ...
    createFantasyWorld() {
        ...
        for (let i = 0; i < 60; i++) {
            ...
            const particle = this.add.circle(x, y, Phaser.Math.Between(1, 3), 0xffd700, 0.8);
        ...
        // Title
        this.add.text(this.scale.width / 2, 50, "👑 QUEST FOR KNOWLEDGE 👑", {
            fontSize: '28px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffd700',
            stroke: '#4B0082',
        ...
        this.realmIndicator = this.add.text(this.scale.width / 2, 80, `🌟 CENTRAL NEXUS`, {
            fontSize: '16px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffffff',
            stroke: '#000000',
    ...
    createMysticalStructures() {
        ...
        // Central nexus
        const nexus = this.add.circle(this.scale.width / 2, this.scale.height / 2, 80, 0x4c1d95);
    ...
    createPlayer() {
        ...
        this.playerHat = this.add.polygon(150, this.scale.height / 2 - 25, [
            0, -15, -8, 0, 8, 0
        ], 0x4B0082);
        this.playerHat.setStrokeStyle(1, 0xffd700);
        
        // Add magical staff
        this.playerStaff = this.add.rectangle(150 + 15, this.scale.height / 2, 3, 25, 0x8B4513);
        const staffOrb = this.add.circle(150 + 15, this.scale.height / 2 - 15, 4, 0xffd700);
        ...
        // Player label
        this.add.text(this.player.x, this.player.y - 45, "HERO", {
            fontSize: '10px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffd700',
            fontStyle: 'bold'
    ...
    createRealms() {
        ...
        this.realms = {
            math: { 
                ...
                color: 0xff6b6b, name: 'Forest of Numbers',
                icon: '🧮', description: 'Solve math riddles'
            },
            reading: { 
                x: 700, y: 180, width: 140, height: 120, 
                color: 0x4ecdc4, name: 'Library of Words',
                icon: '📖', description: 'Reading comprehension'
            },
            science: { 
                x: 180, y: 400, width: 130, height: 110, 
                color: 0x45b7d1, name: 'Wizard\'s Laboratory',
                icon: '🔬', description: 'Scientific experiments'
            },
            history: { 
                x: 720, y: 420, width: 125, height: 105, 
                color: 0xf39c12, name: 'Ancient Temple',
                icon: '🏛️', description: 'Historical mysteries'
            }
        };
        ...
        Object.entries(this.realms).forEach(([realmName, realm]) => {
            // Realm portal
            const portal = this.add.ellipse(realm.x, realm.y, realm.width, realm.height, realm.color, 0.6);
            portal.setStrokeStyle(3, 0xffd700);
            
            // Realm icon
            this.add.text(realm.x, realm.y - 20, realm.icon, {
                fontSize: '24px'
            }).setOrigin(0.5);
            
            // Realm name
            this.add.text(realm.x, realm.y + 20, realm.name, {
                fontSize: '12px',
                fontFamily: 'Arial, sans-serif',
                fill: '#ffffff',
                fontStyle: 'bold',
    ...
    createCrownPieces() {
        ...
        const crownData = [
            { x: 200, y: 200, subject: 'math', color: 0xff6b6b, name: 'Ruby Crown Piece' },
            { x: 700, y: 180, subject: 'reading', color: 0x4ecdc4, name: 'Emerald Crown Piece' },
            { x: 180, y: 400, subject: 'science', color: 0x45b7d1, name: 'Sapphire Crown Piece' },
            { x: 720, y: 420, subject: 'history', color: 0xf39c12, name: 'Golden Crown Piece' }
        ];
        
        crownData.forEach(crownInfo => {
            // Create physics body using a graphics object instead of null sprite
            const crownPiece = this.physics.add.existing(this.add.polygon(crownInfo.x, crownInfo.y, [
            ...
            // Add sparkle effect
            const sparkle = this.add.text(crownInfo.x, crownInfo.y, '✨', {
                fontSize: '16px'
            }).setOrigin(0.5);
    ...
    createGuardians() {
        ...
        const guardianData = [
            ...
            { x: 180, y: 360, subject: 'science', icon: '🧙', name: 'Science Wizard' },
            { x: 720, y: 380, subject: 'history', icon: '⚔️', name: 'Ancient Guardian' }
        ];
        
        guardianData.forEach(guardianInfo => {
            // Create physics body using a graphics object instead of null sprite
            const guardian = this.physics.add.existing(this.add.circle(guardianInfo.x, guardianInfo.y, 20, 0x4b0082));
            guardian.setStrokeStyle(2, 0xffd700);
            guardian.body.setCircle(20);
            
            // Guardian visual
            const guardianIcon = this.add.text(guardianInfo.x, guardianInfo.y, guardianInfo.icon, {
                fontSize: '20px'
            }).setOrigin(0.5);
            
            // Guardian name
            const guardianLabel = this.add.text(guardianInfo.x, guardianInfo.y + 25, guardianInfo.name, {
                fontSize: '10px',
                fontFamily: 'Arial, sans-serif',
                fill: '#ffd700',
                fontStyle: 'bold'
    ...
    createUI() {
        ...
        this.crownText = this.add.text(20, 110, `Crown Pieces: ${this.crownPiecesCollected}/${this.totalCrownPieces}`, {
            fontSize: '16px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffd700',
            stroke: '#000000',
        ...
        this.levelText = this.add.text(20, 140, `Level: ${this.playerLevel} | XP: ${this.experience}`, {
            fontSize: '14px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffffff',
            stroke: '#000000',
        ...
        this.finalBossBtn = this.add.rectangle(this.scale.width / 2, this.scale.height - 40, 200, 30, 0x8B0000)
            .setStrokeStyle(2, 0xffd700)
            .setInteractive({ useHandCursor: true })
            .setVisible(false);
        
        this.finalBossText = this.add.text(this.scale.width / 2, this.scale.height - 40, "⚔️ FACE THE FINAL BOSS", {
            fontSize: '14px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffd700',
            fontStyle: 'bold'
    ...
    setupPhysics() {
        ...
        this.physics.add.overlap(this.player, this.guardians, this.challengeGuardian, null, this);
        
        // Player collects crown pieces
        this.physics.add.overlap(this.player, this.crownPieces, this.collectCrownPiece, null, this);
    }
    ...
    updateCurrentRealm() {
        ...
        Object.entries(this.realms).forEach(([realmName, realm]) => {
            const distance = Phaser.Math.Distance.Between(this.player.x, this.player.y, realm.x, realm.y);
            if (distance < 60) {
                inRealm = realmName;
            }
        });
    ...
    showGuardianChallenge(guardian) {
        ...
        this.add.text(this.scale.width / 2, yPos, `⚔️ GUARDIAN CHALLENGE ⚔️`, {
            fontSize: '24px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffd700',
            fontStyle: 'bold'
        ...
        yPos += 40;
        this.add.text(this.scale.width / 2, yPos, `${guardian.icon} ${guardian.name} ${guardian.icon}`, {
            fontSize: '20px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffffff'
        }).setOrigin(0.5);
        ...
        choices.forEach((choice, index) => {
            ...
            const button = this.add.rectangle(x, yPos, buttonWidth, buttonHeight, 0x8B0000)
                .setStrokeStyle(2, 0xffd700)
                .setInteractive({ useHandCursor: true });
    ...
    collectCrownPiece(player, crownPiece) {
        if (crownPiece.collected || !this.questsCompleted[crownPiece.subject]) {
            if (!this.questsCompleted[crownPiece.subject]) {
                this.showFeedback(`Defeat the ${crownPiece.subject} guardian first!`, 0xef4444);
            }
            return;
        }
        ...
        this.sessionCoinsText.setText(`Session: +${this.coinsEarned} 🪙`);
        this.scoreText.setText(`Score: ${this.score}`);
        this.crownText.setText(`Crown Pieces: ${this.crownPiecesCollected}/${this.totalCrownPieces}`);
        this.levelText.setText(`Level: ${this.playerLevel} | XP: ${this.experience}`);
        
        // Show feedback
        this.showFeedback(`👑 ${crownPiece.name} Collected! +200 points, +${coinsEarned} coins`, 0xffd700);
    ...
    unlockFinalBoss() {
        ...
        this.showFeedback("🏆 All Guardians Defeated! Final Boss Unlocked!", 0xffd700);
    }
    ...
    startFinalBoss() {
        ...
        const overlay = this.add.rectangle(0, 0, this.scale.width, this.scale.height, 0x8B0000, 0.9)
            .setOrigin(0, 0);
        ...
        this.add.text(this.scale.width / 2, yPos, "🐲 FINAL BOSS BATTLE! 🐲", {
            fontSize: '28px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ff0000',
            fontStyle: 'bold'
    ...
    endGame(victory) {
        ...
        if (victory) {
            this.add.text(this.scale.width / 2, yPos, "🏆 KNOWLEDGE MASTER! 🏆", {
                fontSize: '28px',
                fontFamily: 'Arial, sans-serif',
                fill: '#ffd700',
                fontStyle: 'bold'
            ...
            this.add.text(this.scale.width / 2, yPos, "👑 Continue Your Quest! 👑", {
                fontSize: '28px',
                fontFamily: 'Arial, sans-serif',
                fill: '#ef4444',
                fontStyle: 'bold'
        ...
        yPos += 30;
        this.add.text(this.scale.width / 2, yPos, `Hero Level: ${this.playerLevel}`, {
            fontSize: '18px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffffff'
        }).setOrigin(0.5);
        ...
        yPos += 30;
        this.add.text(this.scale.width / 2, yPos, `Crown Pieces: ${this.crownPiecesCollected}/${this.totalCrownPieces}`, {
            fontSize: '18px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffd700'
        }).setOrigin(0.5);
        ...
        yPos += 30;
        const completedQuests = Object.values(this.questsCompleted).filter(q => q).length;
        this.add.text(this.scale.width / 2, yPos, `Quests Completed: ${completedQuests}/4`, {
            fontSize: '18px',
            fontFamily: 'Arial, sans-serif',
            fill: '#ffffff'
        }).setOrigin(0.5);
