...
export class PixelArtGenerator {
  ...
  initializeAdvancedSystems() {
    try {
      ...
      console.error(
        'PixelArtGenerator: Error initializing advanced systems:',
        error
      );
    }
  }
  ...
  initializeTextureAtlas() {
    try {
      ...
      if (!this.textureAtlas.context) {
        console.error(
          'PixelArtGenerator: Failed to get 2D context for texture atlas'
        );
      ...
      console.error(
        'PixelArtGenerator: Error initializing texture atlas:',
        error
      );
      this.textureAtlas = null;
    }
  ...
  generateAdvancedCharacterSprite(characterType, config = {}) {
    try {
      ...
      if (!this.scene) {
        console.error(
          'PixelArtGenerator: Scene not available for sprite generation'
        );
      ...
      const spriteData = this.createAdvancedSpriteData(
        characterType,
        characterDNA,
        fullConfig
      );
      ...
      if (!spriteData) {
        console.error(
          `PixelArtGenerator: Failed to create advanced sprite data for ${characterType}`
        );
        return this.createFallbackSprite(characterType);
      }

      // Create texture with advanced features
      const textureKey = this.createAdvancedTexture(
        spriteData,
        cacheKey,
        fullConfig
      );
      ...
      if (!textureKey) {
        console.error(
          `PixelArtGenerator: Failed to create advanced texture for ${characterType}`
        );
        return this.createFallbackSprite(characterType);
      }

      // Create advanced animations
      const animationKeys = this.createAdvancedAnimations(
        textureKey,
        characterType,
        fullConfig
      );
      ...
      console.log(
        `PixelArtGenerator: Successfully created advanced sprite: ${characterType}`
      );
      return result;
    } catch (error) {
      console.error(
        `PixelArtGenerator: Error generating advanced character sprite:`,
        error
      );
      return this.createFallbackSprite(characterType);
    }
  ...
  createAdvancedSpriteData(characterType, dna, config) {
    try {
      const artStyle =
        this.artStyles[config.artStyle] || this.artStyles.modern16bit;
      ...
      if (config.equipment) {
        spriteData.layers.push(
          ...this.generateEquipmentLayers(config.equipment, dna, palette)
        );
      }

      // Add effect layers
      if (config.effects && config.effects.length > 0) {
        spriteData.effects.push(
          ...this.generateEffectLayers(config.effects, dna)
        );
      }
      ...
      console.error(
        'PixelArtGenerator: Error creating advanced sprite data:',
        error
      );
      return null;
    }
  ...
  createAdvancedFrame(
    ...
    try {
      ...
      if (!ctx) {
        console.error(
          'PixelArtGenerator: Failed to get 2D context for frame canvas'
        );
        return null;
      }
      ...
      // Draw base character
      this.drawAdvancedCharacterBase(
        ctx,
        characterType,
        dna,
        palette,
  ...
  createAdvancedTexture(spriteData, textureKey, config) {
    try {
      ...
      if (!this.textureAtlas || !this.textureAtlas.context) {
        console.warn(
          'PixelArtGenerator: Texture atlas not available, using individual texture'
        );
      ...
      console.error(
        'PixelArtGenerator: Error creating advanced texture:',
        error
      );
      return null;
    }
  ...
  createAdvancedAnimations(textureKey, characterType, config) {
    ...
    try {
      ...
      console.error(
        'PixelArtGenerator: Error creating advanced animations:',
        error
      );
      return {};
    }
  ...
  generateAdvancedEnemySprite(enemyType, difficulty = 1, config = {}) {
    ...
    return this.generateAdvancedCharacterSprite(
      `enemy_${enemyType}`,
      enhancedConfig
    );
  }
  ...
  createFallbackSprite(characterType) {
    try {
      ...
      if (!ctx) {
        console.error(
          'PixelArtGenerator: Failed to get 2D context for fallback sprite'
        );
        return {
          textureKey: null,
          animationKeys: {},
      ...
      if (!this.scene || !this.scene.textures) {
        console.error(
          'PixelArtGenerator: Scene or textures not available for fallback sprite'
        );
        return {
          textureKey: null,
          animationKeys: {},
      ...
      console.error(
        'PixelArtGenerator: Error creating fallback sprite:',
        error
      );
      return {
        textureKey: null,
  ...
  drawAriaBodyDetails(ctx, x, y, dna, palette) {
    ...
    this.drawAdvancedPixelRect(ctx, x + 2, y + 4, 4, 2, palette.secondary, {
      glow: true,
    });
    this.drawAdvancedPixelRect(ctx, x + 10, y + 4, 4, 2, palette.secondary, {
      glow: true,
    });
  ...
  drawTitanBodyDetails(ctx, x, y, dna, palette) {
    ...
    // Heavy armor plating
    this.drawAdvancedPixelRect(ctx, x - 2, y, 20, 4, palette.dark, {
      metallic: true,
    });
    this.drawAdvancedPixelRect(ctx, x - 1, y + 16, 18, 3, palette.dark, {
      metallic: true,
    });
  ...
  drawFacialFeatures(ctx, x, y, dna, palette) {
    ...
    if (faceType % 3 === 0) {
      this.drawAdvancedPixelRect(ctx, x + 6, eyeY, 2, 2, palette.energy, {
        glow: true,
      });
      this.drawAdvancedPixelRect(ctx, x + 16, eyeY, 2, 2, palette.energy, {
        glow: true,
      });
    }
  ...
  generateEquipmentLayers(equipment, dna, palette) {
    return [];
  }

  generateEffectLayers(effects, dna) {
    return [];
  }

  calculateOptimalTextureDimensions(spriteData) {
    return { width: 256, height: 256 };
  }

  addToTextureAtlas(spriteData, dimensions) {
    return null;
  }

  createIndividualTexture(spriteData, textureKey) {
    return textureKey;
  }

  addAdvancedFrameDefinitions(textureKey, spriteData, atlasRegion) {
    /* Implementation */
  }

  generateAnimationFrames(textureKey, stateName, stateConfig) {
    return [];
  }

  calculateFrameRate(stateConfig) {
    return Math.floor(stateConfig.speed * 8);
  }

  shouldYoyo(stateName) {
    return ['idle', 'defend'].includes(stateName);
  }
  ...
  createTransitionAnimations(textureKey, characterType, animationKeys) {
    /* Implementation */
  }

  calculateThreatLevel(difficulty) {
    return Math.min(10, Math.floor(difficulty * 2));
  }

  generateAIPersonality(enemyType, difficulty) {
    return {
      aggression:
        enemyType === 'destroyer' ? 0.9 : enemyType === 'warrior' ? 0.7 : 0.4,
  ...
  calculateAtlasUsage() {
    return (this.textureAtlas.currentY / this.textureAtlasSize) * 100;
  }

  estimateAdvancedMemoryUsage() {
    const baseSize =
      this.config.baseSize.width * this.config.baseSize.height * 4;
    return (
      (this.spriteCache.size * baseSize * this.config.animationFrames) /
      1024 /
      1024
    );
  }
  ...
  createGlowShader() {
    return { type: 'glow', intensity: 1.0 };
  }

  createOutlineShader() {
    return { type: 'outline', thickness: 1 };
  }

  createChromaticShader() {
    return { type: 'chromatic', offset: 2 };
  }

  createPixelateShader() {
    return { type: 'pixelate', size: 4 };
  }

  createHologramShader() {
    return { type: 'hologram', flicker: 0.1 };
  }
  ...
  generateCharacterSprite(characterType, config = {}) {
    console.warn(
      'PixelArtGenerator: generateCharacterSprite is deprecated. Use generateAdvancedCharacterSprite instead.'
    );
    ...
    const result = this.generateAdvancedCharacterSprite(
      characterType,
      advancedConfig
    );
  ...
  generateEnemySprite(enemyType, difficulty = 1) {
    console.warn(
      'PixelArtGenerator: generateEnemySprite is deprecated. Use generateAdvancedEnemySprite instead.'
    );
  ...
  createCharacterAnimations(textureKey, characterType) {
    console.warn(
      'PixelArtGenerator: createCharacterAnimations is deprecated. Animations are now created automatically.'
    );
    ...
    try {
      ...
      console.error(
        'PixelArtGenerator: Error creating fallback animation:',
        error
      );
      return null;
    }
