export class AdaptiveDifficultyManager {
  ...
  getCurrentDifficulty(subject) {
    ...
    if (accuracy >= 0.8 + difficultyModifier) {
      return this.difficultyLevels.hard;
    }
    if (accuracy >= 0.7 + difficultyModifier) {
  ...
  adjustDifficultyDynamic(subject, recentAnswers) {
    ...
    const avgResponseTime =
      recentAnswers.reduce((sum, answer) => sum + answer.responseTime, 0) /
    ...
    if (
      recentAccuracy >= 0.8 &&
      avgResponseTime < this.adaptiveSettings.responseTimeTarget
    ) {
    ...
    // Decrease difficulty if struggling
      ...
      recentAccuracy < 0.5 ||
      avgResponseTime > this.adaptiveSettings.responseTimeTarget * 2
    ) {
      newDifficultyId = this.getNextDifficultyLevel(currentDifficulty.id, -1);
    }
  ...
  calculateRewards(difficulty, responseTime, isCorrect) {
    ...
    if (characterType?.bonusMultipliers.experience) {
      experience = Math.floor(
        experience * characterType.bonusMultipliers.experience
      );
    }
  ...
  recordAnswer(subject, question, userAnswer, correctAnswer, responseTime) {
    ...
    const totalTime =
      this.sessionPerformance.averageResponseTime *
        (this.sessionPerformance.questionsAnswered - 1) +
      responseTime;
    this.sessionPerformance.averageResponseTime =
      totalTime / this.sessionPerformance.questionsAnswered;
  ...
  addToStrugglingTopics(topic) {
    const existing = this.sessionPerformance.strugglingTopics.find(
      t => t.topic === topic
    );
    if (existing) {
      existing.count++;
  ...
  addToStrongTopics(topic) {
    const existing = this.sessionPerformance.strongTopics.find(
      t => t.topic === topic
    );
    if (existing) {
      existing.count++;
  ...
  getSessionSummary() {
    const accuracy =
      this.sessionPerformance.questionsAnswered > 0
        ? this.sessionPerformance.correctAnswers /
          this.sessionPerformance.questionsAnswered
        : 0;
    ...
    return {
      ...
      averageResponseTime: Math.round(
        this.sessionPerformance.averageResponseTime / 1000
      ),
