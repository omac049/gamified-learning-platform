export class EventManager {
  ...
  shouldTriggerEvent(event, eventData) {
    ...
    switch (condition.type) {
      ...
      case 'accuracy':
        if (progress.sessionStats.questionsAnswered < condition.questions)
          return false;
        const accuracy =
      ...
      case 'subject_accuracy':
        const { subjectStats } = eventData;
        if (!subjectStats || subjectStats.total < condition.questions)
          return false;
  ...
  updateChallengeProgress(activation, eventData) {
    ...
    if (event.effects.timeLimit) {
      activation.progress.timeRemaining = Math.max(
        0,
  ...
  isChallengeCompleted(activation) {
    const { event } = activation;
    const { progress } = activation;

    // Check question requirements
    if (
      event.effects.questionsRequired &&
  ...
  openMysteryBox(activation) {
    const { event } = activation;
    const { possibleRewards } = event.effects;
    const reward =
  ...
  getActiveEffects() {
    ...
    this.activeEvents.forEach(activation => {
      ...
      if (event.effects.powerUpDropRate) {
        effects.powerUpDropRate = Math.max(
          effects.powerUpDropRate,
          event.effects.powerUpDropRate
        );
      }
  ...
  getEventStats() {
    return {
      ...
      successRate:
        (this.eventStats.challengesCompleted /
          Math.max(1, this.eventStats.eventsTriggered)) *
        100,
    };
  }
