...
export class ProgressTracker {
    constructor() {
        ...
        this.characterTypes = {
            aria: {
                ...
                specialAbilities: ['Neural Override', 'Cyber Analysis', 'Data Processing']
            },
            titan: {
                id: 'titan',
                name: 'TITAN',
                ...
                specialAbilities: ['Berserker Mode', 'Heavy Assault', 'Defensive Matrix']
            },
            nexus: {
                id: 'nexus',
                name: 'NEXUS',
                ...
                specialAbilities: ['Quantum Sync', 'Tech Innovation', 'System Analysis']
            }
        };
    ...
    loadProgress() {
        ...
        if (savedData) {
            console.log('ProgressTracker: Loaded existing progress for', savedData.character?.name || 'Unknown Player');
    ...
    migrateProgressData(data) {
        ...
        if (!data.subjectStats) {
            ...
            console.log('ProgressTracker: Migrated save data to include subjectStats');
        }
        ...
        if (!data.subjectAccuracies) {
            ...
            console.log('ProgressTracker: Migrated save data to include subjectAccuracies');
        }
    ...
    applyCharacterStartingBonuses(characterData) {
        ...
        if (charType.bonusMultipliers.coins) {
            this.progress.coinBalance = Math.floor(this.progress.coinBalance * charType.bonusMultipliers.coins);
        ...
        if (charType.bonusMultipliers.experience) {
            this.progress.experienceMultiplier *= charType.bonusMultipliers.experience;
        }
    ...
    getPlayerName() {
        return this.progress.character?.name || this.progress.playerName || "Young Scholar";
    }
    ...
    awardCoins(amount, reason = "Achievement") {
        ...
        console.log(`ProgressTracker: Awarded ${finalAmount} coins for ${reason} (${multiplier.toFixed(2)}x multiplier)`);
        this.saveProgress();
        return finalAmount;
    }
    ...
    checkCharacterLevelUp() {
        ...
        if (currentExp >= expNeeded) {
            ...
            this.progress.coinBalance += 50 * this.progress.characterProgression.level;
            this.progress.totalCoinsEarned += 50 * this.progress.characterProgression.level;
            
            console.log(`ProgressTracker: Character leveled up to ${this.progress.characterProgression.level}!`);
            return true;
        }
        return false;
    }
    ...
    recordAnswer(subject, isCorrect, timeSpent = 0, difficulty = 'medium') {
        ...
        if (isCorrect) {
            ...
            const coinReward = this.calculateCoinReward(subject, difficulty, timeSpent);
            this.awardCoins(coinReward, `Correct ${subject} answer`);
            
            // Award character experience
            this.awardCharacterExperience(this.calculateExperienceReward(subject, difficulty));
        }
    ...
    calculateCoinReward(subject, difficulty, timeSpent) {
        ...
        switch(difficulty) {
            case 'easy': baseReward = 3; break;
            case 'medium': baseReward = 5; break;
            case 'hard': baseReward = 8; break;
        }
        
        // Time bonus (faster answers get slight bonus)
        if (timeSpent > 0 && timeSpent < 10000) { // Less than 10 seconds
            baseReward += 2;
        }
    ...
    calculateExperienceReward(subject, difficulty) {
        ...
        switch(difficulty) {
            case 'easy': baseExp = 8; break;
            case 'medium': baseExp = 10; break;
            case 'hard': baseExp = 15; break;
        }
    ...
    updateAccuracies() {
        ...
        if (!this.progress.subjectStats || typeof this.progress.subjectStats !== 'object') {
            return;
        }
        if (!this.progress.subjectAccuracies || typeof this.progress.subjectAccuracies !== 'object') {
        ...
        Object.keys(this.progress.subjectStats).forEach(subject => {
            ...
            if (stats && typeof stats === 'object' && stats.total > 0) {
                this.progress.subjectAccuracies[subject] = Math.round((stats.correct / stats.total) * 100);
            }
        });
    }
    ...
    completeWeek(weekNumber) {
        if (!this.progress.weeksCompleted.includes(weekNumber)) {
            ...
            console.log(`ProgressTracker: Week ${weekNumber} completed! Awarded ${bonusCoins} bonus coins.`);
        }
        this.saveProgress();
    }
    ...
    getEquippedEffects() {
        ...
        switch(equipped.tool) {
            case 'speedBoost': effects.speedMultiplier *= 1.5; break;
            case 'jumpBoost': effects.jumpMultiplier *= 1.3; break;
            case 'scoreMultiplier': effects.scoreMultiplier *= 1.5; break;
            case 'coinMagnet': effects.coinMagnet = true; break;
            case 'autoHint': effects.autoHint = true; break;
        }
        
        // Apply power-up effects
        switch(equipped.powerUp) {
            case 'extraTime': effects.extraTime = 30; break;
            case 'shield': effects.shield = true; break;
            case 'slowMotion': effects.extraTime = 15; break;
        }
    ...
    claimDailyReward() {
        ...
        const lastClaimed = this.progress.dailyRewards.lastClaimed;
        ...
        const reward = {
            ...
            isNewRecord: this.progress.dailyRewards.streak > (this.progress.dailyRewards.maxStreak || 0)
        };
    ...
    getProgressSummary() {
        ...
        return {
            ...
            expNeededForNextLevel: this.getExpNeededForLevel(this.getCharacterLevel() + 1),
    ...
    purchaseItem(itemId, itemType, cost) {
        if (this.spendCoins(cost)) {
            ...
            if (itemType === 'powerUp') {
                if (typeof this.progress.inventory[itemType + 's'][itemId] === 'number') {
    ...
    hasArmorUpgrade(armorId) {
        return !!(this.progress.armorUpgrades && this.progress.armorUpgrades[armorId]);
    }
    ...
    getEquippedArmorLevel(mechType, armorType) {
        ...
        Object.keys(this.progress.armorUpgrades).forEach(armorId => {
            if (this.progress.armorUpgrades[armorId] && armorId.startsWith(`${mechType}_${armorType}_`)) {
    ...
    usePowerUp(itemId) {
        ...
        if (!this.progress.inventory.powerUps || !this.progress.inventory.powerUps[itemId]) {
            return false;
        }
    ...
    getInventorySummary() {
        ...
        if (this.progress.inventory.powerUps) {
            Object.entries(this.progress.inventory.powerUps).forEach(([itemId, value]) => {
        ...
        ['cosmetics', 'tools', 'decorations'].forEach(category => {
            if (this.progress.inventory[category]) {
                Object.entries(this.progress.inventory[category]).forEach(([itemId, owned]) => {
                    if (owned) {
    ...
    equipItem(itemId, itemType) {
        if (this.progress.inventory[itemType + 's'] && this.progress.inventory[itemType + 's'][itemId]) {
            this.progress.equippedItems[itemType] = itemId;
            this.saveProgress();
            return true;
        }
    ...
    unlockCharacterUpgrade(upgradeId) {
        if (!this.progress.characterProgression.upgradesUnlocked.includes(upgradeId)) {
    ...
    isCharacterUpgradeUnlocked(upgradeId) {
        return this.progress.characterProgression.upgradesUnlocked.includes(upgradeId);
    }
