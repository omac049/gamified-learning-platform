export class QuestionManager {
  constructor() {
    ...
    this.readingQuestions = [
      {
        passage:
          'The cat sat on the mat. It was a sunny day. The cat liked to watch birds fly by the window.',
      ...
      {
        passage:
          'Birds can fly high in the sky. They have wings and feathers. Birds build nests to keep their eggs safe.',
      ...
      {
        passage:
          'Sarah loves to read books. Every night before bed, she reads for 30 minutes. Her favorite books are about adventures.',
      ...
      {
        passage:
          'The library is a quiet place where people go to read and study. You can borrow books for free with a library card.',
      ...
      {
        passage:
          'Dogs are loyal pets. They love to play fetch and go for walks. Dogs wag their tails when they are happy.',
      ...
      {
        passage:
          'The ocean is home to many sea creatures. Fish, whales, and dolphins all live in the salty water. Coral reefs provide shelter for small fish.',
      ...
      {
        passage:
          'Bees are important insects. They help flowers grow by carrying pollen from one flower to another. Bees also make honey.',
    ...
    this.historyQuestions = [
      ...
      {
        question:
          'Which ocean did Christopher Columbus cross to reach America?',
      ...
      {
        question:
          'What did the Pilgrims celebrate with the first Thanksgiving?',
      ...
      {
        question:
          "What do we call the 13 original colonies' fight for independence?",
      ...
      {
        question:
          'Who helped Lewis and Clark explore the western United States?',
        answer: 'Sacagawea',
  ...
  getRandomReadingQuestion() {
    return this.readingQuestions[
      Math.floor(Math.random() * this.readingQuestions.length)
    ];
  }
  ...
  getRandomHistoryQuestion() {
    return this.historyQuestions[
      Math.floor(Math.random() * this.historyQuestions.length)
    ];
  }

  getRandomVocabularyQuestion() {
    return this.vocabularyQuestions[
      Math.floor(Math.random() * this.vocabularyQuestions.length)
    ];
  }
  ...
  getQuestionsByDifficulty(subject, difficulty = 'medium') {
    ...
    switch (subject) {
      case 'math':
        ...
        if (difficulty === 'hard') {
          return this.mathQuestions.filter(
            q => q.type === 'multiplication' || q.type === 'division'
          );
        }
  ...
  getQuestionCount(subject) {
    switch (subject) {
      case 'math':
        return this.mathQuestions.length;
      case 'reading':
        return this.readingQuestions.length;
      case 'science':
        return this.scienceQuestions.length;
      case 'history':
        return this.historyQuestions.length;
      case 'vocabulary':
    ...
    switch (subject.toLowerCase()) {
      case 'math':
        ...
        if (difficulty === 'Easy') {
          ...
          const simpleSubtraction = this.mathQuestions.filter(
            q =>
              q.type === 'subtraction' &&
              parseInt(q.question.split(' - ')[0]) <= 15
          );
          const additionQuestions = this.mathQuestions.filter(
            q => q.type === 'addition'
          );
          ...
          const selectedType =
            hardTypes[Math.floor(Math.random() * hardTypes.length)];
          rawQuestion = this.getRandomMathQuestion(selectedType);
        } else if (difficulty === 'Expert') {
          // Expert: All types with emphasis on multiplication and division
          const expertQuestions = this.mathQuestions.filter(
            q => q.type === 'multiplication' || q.type === 'division'
          );
          // Add some challenging addition/subtraction
          const challengingArithmetic = this.mathQuestions.filter(
            q =>
              (q.type === 'addition' &&
                parseInt(q.question.split(' + ')[0]) >= 12) ||
              (q.type === 'subtraction' &&
                parseInt(q.question.split(' - ')[0]) >= 18)
          );
          const allExpertQuestions = [
            ...expertQuestions,
            ...challengingArithmetic,
          ];
          rawQuestion =
            allExpertQuestions[
              Math.floor(Math.random() * allExpertQuestions.length)
            ];
        } else {
