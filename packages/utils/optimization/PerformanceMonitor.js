...
export class PerformanceMonitor {
  ...
  createPerformanceDisplay() {
    ...
    if (this.config.showFPS) {
      ...
      this.displayElements.avgFpsText = this.scene.add.text(
        x,
        y + 16,
        'Avg: --',
        {
    ...
    if (this.config.showMemory) {
      this.displayElements.memoryText = this.scene.add.text(
        x,
        y + 35,
        'Memory: --',
        {
    ...
    if (this.config.showRenderStats) {
      this.displayElements.renderText = this.scene.add.text(
        x,
        y + 50,
        'Draws: --',
        {
  ...
  updateMemoryUsage() {
    if (performance.memory) {
      this.memoryUsage.used = Math.round(
        performance.memory.usedJSHeapSize / 1024 / 1024
      );
      this.memoryUsage.total = Math.round(
        performance.memory.totalJSHeapSize / 1024 / 1024
      );
      this.memoryUsage.percentage = Math.round(
        (this.memoryUsage.used / this.memoryUsage.total) * 100
      );
    }
  }
  ...
  logPerformanceReport() {
    ...
    console.log(
      `FPS: ${report.fps.current} (avg: ${report.fps.average}, range: ${report.fps.min}-${report.fps.max})`
    );
    console.log(
      `Memory: ${report.memory.used}MB / ${report.memory.total}MB (${report.memory.percentage}%)`
    );
    console.log(
      `Render: ${report.renderStats.drawCalls} draws, ${report.renderStats.textureSwaps} texture swaps`
    );
