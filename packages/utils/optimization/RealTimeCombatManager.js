...
export class RealTimeCombatManager {
  ...
  endCombat() {
    ...
    this.combatStats.accuracyRate =
      this.combatStats.shotsFired > 0
  ...
  fireWeapon(weaponType, targetX, targetY) {
    ...
    if (!this.scene || !this.scene.playerMech) {
      console.error('fireWeapon: scene or playerMech not available');
      return false;
    }
    ...
    // Calculate accuracy
    const accuracy =
      weapon.accuracy * (this.activeAbilities.has('rapidFire') ? 0.9 : 1);
    ...
    for (let i = 0; i < projectileCount; i++) {
      let angle = Math.atan2(
        targetY - this.scene.playerMech.y,
        targetX - this.scene.playerMech.x
      );

      if (projectileCount > 1) {
        const spreadOffset =
          (i - (projectileCount - 1) / 2) * (spread / projectileCount);
        angle += spreadOffset;
      }
  ...
  createProjectile(weapon, angle, willHit) {
    // Validate required dependencies
    if (!this.scene || !this.scene.playerMech) {
      console.error('createProjectile: scene or playerMech not available');
      return;
    }
    ...
    if (weapon.effects && weapon.effects.includes('bulletTrail')) {
  ...
  createWeaponEffects(weapon, targetX, targetY) {
    // Validate required dependencies
    if (!this.scene || !this.scene.playerMech) {
      console.error('createWeaponEffects: scene or playerMech not available');
      return;
    }

    if (!weapon || !weapon.effects) {
      console.warn(
        'createWeaponEffects: weapon or weapon.effects not available'
      );
      return;
    }
...
class BulletProjectile extends Phaser.GameObjects.Graphics {
  ...
  initialize(x, y, config) {
    ...
    if (!config) {
      console.error(
        'BulletProjectile.initialize: config parameter is required'
      );
      return;
    }

    if (typeof config.angle === 'undefined') {
      console.error('BulletProjectile.initialize: config.angle is required');
      return;
    }
...
class LaserBeam extends Phaser.GameObjects.Graphics {
  ...
  initialize(startX, startY, endX, endY, config) {
    ...
    if (
      ...
      console.error(
        'LaserBeam.initialize: startX, startY, endX, endY parameters are required'
      );
      return;
    }

    if (!config) {
      console.error('LaserBeam.initialize: config parameter is required');
      return;
    }
...
class CombatEffect extends Phaser.GameObjects.Graphics {
  ...
  initialize(x, y, config) {
    ...
    if (typeof x === 'undefined' || typeof y === 'undefined') {
      console.error('CombatEffect.initialize: x, y parameters are required');
      return;
    }

    if (!config) {
      console.error('CombatEffect.initialize: config parameter is required');
      return;
    }
