...
export class AudioSystem extends GameSystem {
    ...
    createSoundNode(soundDef, options) {
        ...
        oscillator.type = options.waveType || this.getWaveTypeForSound(soundDef.type);
        ...
        const volume = (options.volume || 1) * this.getVolumeForSoundType(soundDef.type);
    ...
    createProceduralMusic(track) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            oscillator.frequency.setValueAtTime(
                baseFrequency * Math.pow(2, i * 0.5),
                this.audioContext.currentTime
            );
