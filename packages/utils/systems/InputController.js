...
export class InputController extends GameSystem {
  ...
  setupTouchControls() {
    // Check if touch is available or if we're on mobile
    const isMobile =
    ...
    if (!hasTouch && !isMobile) {
      this.log('Touch input not available - desktop mode');
      return;
    }

    this.touchEnabled = true;
    this.log('Setting up touch controls for mobile...');
    ...
    this.createTouchAbilityButtons();

    // Create pause button
    this.createTouchPauseButton();
  ...
  createTouchAbilityButtons() {
    const abilities = [
      { key: 'ability1', icon: '⚡', color: 0xffff00 },
      { key: 'ability2', icon: '🛡️', color: 0x00ff00 },
      { key: 'ability3', icon: '🚀', color: 0xff00ff },
    ];

    abilities.forEach((ability, index) => {
      const x = this.scene.cameras.main.width - 100;
      const y = this.scene.cameras.main.height - 200 - index * 60;
      const radius = 25;

      const button = this.scene.add.circle(x, y, radius, ability.color, 0.6);
      button.setDepth(1100);
      button.setScrollFactor(0);
      button.setInteractive();

      const icon = this.scene.add
        .text(x, y, ability.icon, {
          fontSize: '16px',
        })
        .setOrigin(0.5)
        .setDepth(1101)
      ...
      button.on('pointerdown', () => {
        this.inputState.abilityRequested = ability.key;
        button.setAlpha(1);
      });
      ...
      this.touchControls.abilityButtons.push({
        button,
        icon,
        key: ability.key,
      });
  ...
  createTouchPauseButton() {
    ...
    const button = this.scene.add.circle(x, y, radius, 0x888888, 0.7);
    button.setDepth(1100);
    button.setScrollFactor(0);
    button.setInteractive();
    ...
    this.touchControls.pauseButton = { button, icon };
  }
  ...
  updateTouchControls() {
    ...
    // Handle ability button requests
    if (this.inputState.abilityRequested) {
      this.emit('abilityRequested', {
        ability: this.inputState.abilityRequested,
        inputMethod: 'touch',
      });
      this.inputState.abilityRequested = null;
    }

    // Update touch control positions for responsive design
  ...
  updateTouchControlPositions() {
    ...
    if (this.touchControls.movementJoystick) {
      const joystickX = Math.min(100, width * 0.1);
      const joystickY = Math.max(height - 100, height * 0.9);

      this.touchControls.movementJoystick.base.setPosition(
        joystickX,
        joystickY
      );
      this.touchControls.movementJoystick.stick.setPosition(
        joystickX,
        joystickY
      );
    }
    ...
    if (this.touchControls.fireButton) {
      const fireX = Math.max(width - 100, width * 0.9);
      const fireY = Math.max(height - 100, height * 0.9);

      this.touchControls.fireButton.button.setPosition(fireX, fireY);
      this.touchControls.fireButton.icon.setPosition(fireX, fireY);
    }

    // Update weapon button position
    if (this.touchControls.weaponButton) {
      const weaponX = Math.max(width - 180, width * 0.82);
      const weaponY = Math.max(height - 100, height * 0.9);

      this.touchControls.weaponButton.button.setPosition(weaponX, weaponY);
      this.touchControls.weaponButton.icon.setPosition(weaponX, weaponY);
    }

    // Update ability buttons positions
    this.touchControls.abilityButtons.forEach((abilityButton, index) => {
      const abilityX = Math.max(width - 100, width * 0.9);
      const abilityY = Math.max(
        height - 200 - index * 60,
        height * 0.75 - index * 60
      );

      abilityButton.button.setPosition(abilityX, abilityY);
      abilityButton.icon.setPosition(abilityX, abilityY);
    });

    // Update pause button position
    if (this.touchControls.pauseButton) {
      const pauseX = Math.max(50, width * 0.05);
      const pauseY = Math.max(50, height * 0.05);

      this.touchControls.pauseButton.button.setPosition(pauseX, pauseY);
      this.touchControls.pauseButton.icon.setPosition(pauseX, pauseY);
    }
  }
  ...
  processInputHistory() {
    ...
    this.inputHistory = this.inputHistory.filter(
      entry => currentTime - entry.timestamp < 2000 // Keep last 2 seconds
    );
  ...
  checkForCombos() {
    ...
    if (this.inputHistory.length >= konamiCode.length) {
      ...
      const matches = recent.every(
        (entry, index) => entry.action === konamiCode[index]
      );
  ...
  switchToNextWeapon() {
    const nextIndex =
      (this.currentWeaponIndex + 1) % this.availableWeapons.length;
  ...
  getInputMetrics() {
    return {
      ...
      weaponSwitchCooldownRemaining: Math.max(
        0,
        GameConfig.timing.weaponSwitchCooldown -
          (Date.now() - this.lastWeaponSwitch)
      ),
