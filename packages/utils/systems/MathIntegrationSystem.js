...
export class MathIntegrationSystem extends GameSystem {
    constructor(scene, config = {}) {
        ...
        this.mathTriggers = {
            enemyEncounter: { enabled: true, chance: 0.3, cooldown: 5000, lastTriggered: 0 },
            lowHealth: { enabled: true, threshold: 0.3, cooldown: 10000, lastTriggered: 0 },
            waveStart: { enabled: true, chance: 0.5, cooldown: 0, lastTriggered: 0 },
            powerUpRequest: { enabled: true, chance: 1.0, cooldown: 2000, lastTriggered: 0 },
            bossEncounter: { enabled: true, chance: 0.8, cooldown: 0, lastTriggered: 0 }
        };
    ...
    handleCorrectAnswer(timeBonus) {
        ...
        if (this.currentQuestion?.triggerType) {
            this.applyContextualCorrectEffects(this.currentQuestion.triggerType, this.currentQuestion.contextData);
        }
        ...
        this.log(`Correct answer! Streak: ${this.streak}, Math Power: ${this.mathPower}`);
    }
    ...
    handleIncorrectAnswer() {
        ...
        if (this.currentQuestion?.triggerType) {
            this.applyContextualIncorrectEffects(this.currentQuestion.triggerType, this.currentQuestion.contextData);
        }
    ...
    applyContextualCorrectEffects(triggerType, contextData) {
        ...
        switch (triggerType) {
            ...
            case 'enemyEncounter':
                ...
                this.activateBonus('damageMultiplier', 5000, { multiplier: effects.damageMultiplier });
                this.emit('showFloatingText', { 
                    text: 'Damage Boost!', 
                    color: '#ff6600',
    ...
    applyContextualIncorrectEffects(triggerType, contextData) {
        ...
        switch (triggerType) {
            ...
            case 'enemyEncounter':
                ...
                this.activateBonus('vulnerability', penalties.vulnerabilityDuration, {});
                break;
    ...
    checkStreakBonuses() {
        ...
        if (streakBonus) {
            this.activateBonus(streakBonus.effect, streakBonus.duration, { streak: this.streak });
    ...
    // === MATH POWER SYSTEM ===
    addMathPower(amount) {
        this.mathPower = Math.max(0, Math.min(this.maxMathPower, this.mathPower + amount));
    ...
    activateMathBonuses(timeBonus) {
        ...
        if (this.mathPower >= 20) {
            this.activateBonus('energyRestore', 3000, { amount: 15 + Math.floor(timeBonus * 10) });
        }
    ...
    adjustDifficulty() {
        const recentPerformance = this.performanceHistory.slice(-this.difficultyAdjustmentThreshold);
    ...
    increaseDifficulty() {
        ...
        for (const topic in this.topicProgression) {
            if (this.topicProgression[topic].mastery >= 0.8) {
                this.topicProgression[topic].level = Math.min(5, this.topicProgression[topic].level + 1);
            }
        }
    ...
    decreaseDifficulty() {
        ...
        for (const topic in this.topicProgression) {
            if (this.topicProgression[topic].mastery <= 0.4) {
                this.topicProgression[topic].level = Math.max(1, this.topicProgression[topic].level - 1);
            }
        }
    ...
    updateTopicMastery(topic, isCorrect) {
        ...
        if (this.topicProgression[topic].mastery >= 0.9 && this.topicProgression[topic].level < 5) {
            this.topicProgression[topic].level++;
            this.emit('topicLevelUp', { topic, level: this.topicProgression[topic].level });
        }
    }
    ...
    getPerformanceStats() {
        return {
            ...
            accuracy: this.questionsAnswered > 0 ? this.correctAnswers / this.questionsAnswered : 0,
