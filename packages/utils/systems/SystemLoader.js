...
export class SystemLoader {
  ...
  async loadAllSystems() {
    ...
    try {
      ...
      console.log(
        `SystemLoader: Loading complete. ${this.loadedSystems}/${this.totalSystems} systems loaded successfully`
      );

      if (this.failedSystems.length > 0) {
        console.warn(
          'SystemLoader: Some systems failed to load:',
          this.failedSystems
        );
      }
      ...
      console.error(
        'SystemLoader: Critical error during system loading:',
        error
      );
  ...
  async loadSystem(systemDef) {
    ...
    for (let attempt = 0; attempt <= this.options.retryAttempts; attempt++) {
      try {
        ...
        const systemInstance = await this.createSystemWithTimeout(
          SystemClass,
          config
        );
        ...
        console.warn(
          `SystemLoader: ${name} failed to load (attempt ${attempt + 1}):`,
          error
        );
        ...
        if (attempt === this.options.retryAttempts) {
          ...
          if (required && !this.options.fallbackMode) {
            throw new Error(
              `Required system ${name} failed to load: ${error.message}`
            );
          }

          console.error(
            `SystemLoader: ${name} failed to load after ${attempt + 1} attempts`
          );
          break;
        }
  ...
  sortSystemsByDependencies() {
    ...
    const visit = systemDef => {
      if (visiting.has(systemDef.name)) {
        throw new Error(
          `Circular dependency detected involving ${systemDef.name}`
        );
      }
  ...
  showLoadingUI() {
    ...
    this.loadingText = this.scene.add
      .text(centerX, centerY - 50, 'Initializing Game Systems...', {
        fontSize: '24px',
    ...
    this.progressBarBg = this.scene.add
      .rectangle(centerX, centerY, 300, 20, 0x333333)
      .setStrokeStyle(2, 0x00ffff)
      .setDepth(10001);

    // Progress bar fill
    this.progressBar = this.scene.add
      .rectangle(centerX - 148, centerY, 0, 16, 0x00ffff)
      .setOrigin(0, 0.5)
      .setDepth(10002);
    ...
    this.statusText = this.scene.add
      .text(centerX, centerY + 60, 'Preparing...', {
        fontSize: '14px',
        fontFamily: 'Courier, monospace',
        color: '#ffff00',
      })
  ...
  updateProgress() {
    ...
    if (this.statusText) {
      this.statusText.setText(
        `${this.loadedSystems}/${this.totalSystems} systems loaded`
      );
    }
  }
  ...
  createFallbackResult() {
    console.warn(
      'SystemLoader: Creating fallback result due to system failures'
    );
