...
export class UIManager extends GameSystem {
    ...
    initializePools() {
        ...
        this.log(`Initialized pools: ${this.damageNumberPool.length} damage numbers`);
    }
    ...
    createCombatUI() {
        ...
        // Health bar
        this.combatUI.healthBar = this.createProgressBar(20, 20, 200, 20, config.colors.danger, 'Health');
        combatContainer.add(this.combatUI.healthBar.container);
        
        // Shield bar
        this.combatUI.shieldBar = this.createProgressBar(20, 50, 200, 15, config.colors.primary, 'Shield');
        combatContainer.add(this.combatUI.shieldBar.container);
        
        // Energy bar
        this.combatUI.energyBar = this.createProgressBar(20, 75, 200, 15, config.colors.warning, 'Energy');
        combatContainer.add(this.combatUI.energyBar.container);
        
        // Math Power bar - NEW
        this.combatUI.mathPowerBar = this.createProgressBar(20, 105, 200, 20, 0x9966ff, 'Math Power');
        combatContainer.add(this.combatUI.mathPowerBar.container);
        
        // Math Streak display - NEW
        this.combatUI.streakDisplay = this.scene.add.text(20, 140, 'Streak: 0', {
        ...
        this.combatUI.bonusesDisplay = this.scene.add.text(20, 165, '', {
        ...
        // Wave display
        this.combatUI.waveDisplay = this.scene.add.text(this.scene.cameras.main.width - 20, 20, 'Wave 1', {
        ...
        // Score display
        this.combatUI.scoreDisplay = this.scene.add.text(this.scene.cameras.main.width - 20, 60, 'Score: 0', {
            fontSize: config.fonts.sizes.medium,
            fontFamily: config.fonts.primary,
            color: '#ffffff'
        }).setOrigin(1, 0);
        ...
        this.combatUI.mathPerformanceDisplay = this.scene.add.text(this.scene.cameras.main.width - 20, 100, 'Accuracy: 0%', {
            fontSize: config.fonts.sizes.small,
            fontFamily: config.fonts.primary,
            color: '#ffff88'
        }).setOrigin(1, 0);
        ...
        // Combo display
        this.combatUI.comboDisplay = this.scene.add.text(this.scene.cameras.main.width / 2, 100, '', {
        ...
        // Weapon display
        this.combatUI.weaponDisplay = this.scene.add.text(20, this.scene.cameras.main.height - 60, 'Weapon: Rapid Fire', {
            fontSize: config.fonts.sizes.medium,
            fontFamily: config.fonts.primary,
            color: '#ffffff'
        }).setOrigin(0, 1);
        ...
        this.combatUI.quickChallengeNotification = this.scene.add.text(
    ...
    createAbilityButtons() {
        ...
        const spacing = 10;
        const startX = this.scene.cameras.main.width - (abilities.length * (buttonWidth + spacing));
        ...
        abilities.forEach((abilityKey, index) => {
            ...
            const button = this.createAbilityButton(x, startY, buttonWidth, buttonHeight, ability, abilityKey);
    ...
    createAbilityButton(x, y, width, height, ability, key) {
        ...
        // Cooldown overlay
        const cooldownOverlay = this.scene.add.rectangle(0, 0, width, height, 0x000000, 0.7);
        cooldownOverlay.setVisible(false);
        
        // Cooldown text
        const cooldownText = this.scene.add.text(0, 10, '', {
            fontSize: '12px',
            fontFamily: GameConfig.ui.fonts.primary,
            color: '#ffffff'
        }).setOrigin(0.5);
    ...
    createMathGauge() {
        ...
        // Background
        this.mathGauge.background = this.scene.add.rectangle(x, y, width, height, 0x333333, 0.8);
        this.mathGauge.background.setStrokeStyle(2, GameConfig.ui.colors.primary);
        
        // Fill
        this.mathGauge.fill = this.scene.add.rectangle(x - width/2, y, 0, height - 4, GameConfig.ui.colors.primary);
        this.mathGauge.fill.setOrigin(0, 0.5);
        
        // Text
        this.mathGauge.text = this.scene.add.text(x, y - 30, 'Math Power', {
        ...
        hudContainer.add([this.mathGauge.background, this.mathGauge.fill, this.mathGauge.text]);
    ...
    createProgressBar(x, y, width, height, color, label) {
        ...
        // Fill
        const fill = this.scene.add.rectangle(-width/2, 0, width, height - 2, color);
        fill.setOrigin(0, 0.5);
        
        // Label
        const labelText = this.scene.add.text(-width/2, -height/2 - 15, label, {
        ...
        // Value text
        const valueText = this.scene.add.text(width/2, -height/2 - 15, '100/100', {
            fontSize: GameConfig.ui.fonts.sizes.small,
            fontFamily: GameConfig.ui.fonts.primary,
            color: '#ffffff'
        }).setOrigin(1, 1);
    ...
    createMathQuizUI() {
        ...
        // Background panel
        const panel = this.scene.add.rectangle(centerX, centerY, 600, 400, 0x000000, 0.9);
        panel.setStrokeStyle(3, GameConfig.ui.colors.primary);
        
        // Question text
        this.mathUI.questionText = this.scene.add.text(centerX, centerY - 100, '', {
        ...
        // Timer bar
        this.mathUI.timerBar = this.createProgressBar(centerX - 200, centerY - 150, 400, 15, GameConfig.ui.colors.warning, 'Time');
        
        // Feedback text
        this.mathUI.feedbackText = this.scene.add.text(centerX, centerY + 150, '', {
            fontSize: GameConfig.ui.fonts.sizes.large,
            fontFamily: GameConfig.ui.fonts.primary,
            color: '#ffffff',
            align: 'center'
        ...
        mathContainer.add([panel, this.mathUI.questionText, this.mathUI.timerBar.container, this.mathUI.feedbackText]);
    }
    ...
    updateProgressBar(progressBar, current, max) {
        ...
        progressBar.fill.setDisplaySize(progressBar.width * percentage, progressBar.fill.height);
    ...
    updateMathGaugeDisplay() {
        if (this.mathGauge.element && this.mathGauge.value !== undefined) {
            ...
            this.mathGauge.fill.setDisplaySize(this.mathGauge.element.width * percentage, this.mathGauge.fill.height);
        }
    }
    ...
    getDamageNumberFromPool() {
        ...
        if (this.damageNumberPool.length < GameConfig.performance.poolSizes.damageNumbers * 2) {
    ...
    onEnemyDefeated(data) {
        this.showDamageNumber(data.enemy.x, data.enemy.y - 30, `+${data.points}`, 'heal');
    }
    ...
    onAbilityUsed(data) {
        ...
        if (button) {
            ...
            button.cooldownEndTime = Date.now() + GameConfig.abilities[data.ability].cooldown;
    ...
    onMathPowerChanged(data) {
        if (this.combatUI.mathPowerBar) {
            this.updateProgressBar(this.combatUI.mathPowerBar, data.current, data.max);
        }
    ...
    onQuickChallengeStarted(data) {
        if (this.combatUI.quickChallengeNotification) {
            this.combatUI.quickChallengeNotification.setText('⚡ QUICK MATH CHALLENGE! ⚡');
    ...
    setMathGaugeValue(value) {
        this.mathGauge.value = Math.max(0, Math.min(this.mathGauge.maxValue, value));
        this.dirtyElements.add('mathGauge');
    }
    ...
    getUIMetrics() {
        return {
            ...this.uiMetrics,
            damageNumbersActive: this.damageNumberPool.filter(dn => dn.visible).length,
    ...
    startQuickChallengeUI(data) {
        ...
        // Background
        const bg = this.scene.add.rectangle(centerX, centerY, 400, 200, 0x000000, 0.8);
        bg.setStrokeStyle(3, 0xff8800);
        
        // Question text
        const questionText = this.scene.add.text(centerX, centerY - 40, data.question, {
            fontSize: '20px',
            fontFamily: GameConfig.ui.fonts.primary,
            color: '#ffffff',
            align: 'center'
        ...
        const startX = centerX - ((data.choices.length * buttonWidth + (data.choices.length - 1) * buttonSpacing) / 2);
        
        data.choices.forEach((choice, index) => {
            const buttonX = startX + (index * (buttonWidth + buttonSpacing)) + (buttonWidth / 2);
            const button = this.createQuickChallengeButton(buttonX, centerY + 20, choice, index);
            overlay.add(button.container);
        });
        
        // Timer bar
        const timerBg = this.scene.add.rectangle(centerX, centerY + 70, 300, 10, 0x333333);
        const timerFill = this.scene.add.rectangle(centerX - 150, centerY + 70, 300, 8, 0xff8800);
    ...
    createQuickChallengeButton(x, y, text, index) {
        ...
        const buttonText = this.scene.add.text(0, 0, text.toString(), {
            fontSize: '16px',
            fontFamily: GameConfig.ui.fonts.primary,
            color: '#ffffff'
        }).setOrigin(0.5);
    ...
    showPowerGainedEffect(powerGained) {
        ...
        const powerText = this.scene.add.text(centerX + 50, centerY, `+${powerGained} Power!`, {
            fontSize: '18px',
            fontFamily: GameConfig.ui.fonts.primary,
            color: '#00ff88',
            stroke: '#000000',
    ...
    showPowerLostEffect(powerLost) {
        ...
        const powerText = this.scene.add.text(centerX + 50, centerY, `-${powerLost} Power`, {
            fontSize: '18px',
            fontFamily: GameConfig.ui.fonts.primary,
            color: '#ff4444',
            stroke: '#000000',
    ...
    createHealthBar() {
        ...
        this.healthBarBg = this.scene.add.rectangle(x, y, width, height, 0x333333)
            .setOrigin(0, 0)
            .setStrokeStyle(2, 0xffffff);
        
        // Health bar fill
        this.healthBarFill = this.scene.add.rectangle(x + 2, y + 2, width - 4, height - 4, 0x00ff00)
            .setOrigin(0, 0);
        
        // Shield bar
        this.shieldBarFill = this.scene.add.rectangle(x + 2, y + 2, width - 4, height - 4, 0x00ffff)
            .setOrigin(0, 0)
            .setAlpha(0.7);
        ...
        // Add to HUD
        this.hudContainer.add([this.healthBarBg, this.healthBarFill, this.shieldBarFill, this.healthText]);
        
        // Labels
        this.scene.add.text(x, y - 15, 'HEALTH', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#ffffff'
        }).setOrigin(0, 0);
    ...
    createMathPowerMeter() {
        ...
        this.mathPowerBg = this.scene.add.rectangle(x, y, width, height, 0x333333)
            .setOrigin(0, 0)
            .setStrokeStyle(2, 0xffff00);
        
        // Math power fill
        this.mathPowerFill = this.scene.add.rectangle(x + 2, y + 2, 0, height - 4, 0xffff00)
            .setOrigin(0, 0);
        
        // Math power text
        this.mathPowerText = this.scene.add.text(x + width/2, y + height/2, '0/100', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#ffffff',
            stroke: '#000000',
        ...
        // Add to HUD
        this.hudContainer.add([this.mathPowerBg, this.mathPowerFill, this.mathPowerText]);
        
        // Label
        this.scene.add.text(x, y - 12, 'MATH POWER', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#ffff00'
        }).setOrigin(0, 0);
    ...
    createWeaponDisplay() {
        ...
        // Weapon background
        this.weaponBg = this.scene.add.rectangle(x, y, 120, 60, 0x333333, 0.8)
            .setOrigin(0, 0)
            .setStrokeStyle(2, 0x00ff00);
        
        // Weapon name
        this.weaponNameText = this.scene.add.text(x + 60, y + 15, 'RAPID FIRE', {
            fontSize: '12px',
            fontFamily: 'Courier, monospace',
            color: '#00ff00',
            stroke: '#000000',
        ...
        // Ammo count
        this.ammoText = this.scene.add.text(x + 60, y + 35, 'AMMO: ∞', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#ffffff'
        }).setOrigin(0.5);
        ...
        // Weapon hotkeys
        this.weaponHotkeysText = this.scene.add.text(x + 60, y + 50, '1-4: WEAPONS', {
            fontSize: '8px',
            fontFamily: 'Courier, monospace',
            color: '#888888'
        }).setOrigin(0.5);
        ...
        // Add to HUD
        this.hudContainer.add([this.weaponBg, this.weaponNameText, this.ammoText, this.weaponHotkeysText]);
    }
    ...
    createStatsDisplay() {
        ...
        // Stats background
        this.statsBg = this.scene.add.rectangle(x, y, 120, 80, 0x333333, 0.8)
            .setOrigin(0, 0)
            .setStrokeStyle(2, 0x00ffff);
        
        // Wave info
        this.waveText = this.scene.add.text(x + 10, y + 10, 'WAVE: 1', {
            fontSize: '12px',
            fontFamily: 'Courier, monospace',
            color: '#00ffff'
        }).setOrigin(0, 0);
        ...
        // Score
        this.scoreText = this.scene.add.text(x + 10, y + 25, 'SCORE: 0', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#ffffff'
        }).setOrigin(0, 0);
        ...
        // Accuracy
        this.accuracyText = this.scene.add.text(x + 10, y + 40, 'ACCURACY: 0%', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#ffffff'
        }).setOrigin(0, 0);
        ...
        // Enemies remaining
        this.enemiesText = this.scene.add.text(x + 10, y + 55, 'ENEMIES: 0', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#ff6666'
        }).setOrigin(0, 0);
        ...
        // Add to HUD
        this.hudContainer.add([this.statsBg, this.waveText, this.scoreText, this.accuracyText, this.enemiesText]);
    }
    ...
    createStreakIndicator() {
        ...
        // Streak background
        this.streakBg = this.scene.add.rectangle(x, y, 150, 25, 0x333333, 0.9)
            .setOrigin(0.5, 0)
            .setStrokeStyle(2, 0xff00ff)
            .setVisible(false);
        
        // Streak text
        this.streakText = this.scene.add.text(x, y + 12, 'STREAK: 0', {
            fontSize: '14px',
            fontFamily: 'Courier, monospace',
            color: '#ff00ff',
            stroke: '#000000',
    ...
    createAbilitiesDisplay() {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            // Ability background
            const bg = this.scene.add.rectangle(x, y, abilitySize, abilitySize, 0x333333, 0.8)
                .setOrigin(0, 0)
                .setStrokeStyle(2, 0x666666);
            
            // Ability icon (placeholder)
            const icon = this.scene.add.text(x + abilitySize/2, y + abilitySize/2, '?', {
                fontSize: '24px',
                fontFamily: 'Courier, monospace',
                color: '#666666'
            }).setOrigin(0.5);
            ...
            // Cooldown overlay
            const cooldown = this.scene.add.rectangle(x, y, abilitySize, abilitySize, 0x000000, 0.7)
                .setOrigin(0, 0)
                .setVisible(false);
            
            // Hotkey label
            const hotkey = this.scene.add.text(x + abilitySize/2, y + abilitySize + 5, `${i + 1}`, {
                fontSize: '10px',
                fontFamily: 'Courier, monospace',
                color: '#888888'
            }).setOrigin(0.5, 0);
    ...
    createMiniMap() {
        ...
        // Mini-map background
        this.miniMapBg = this.scene.add.rectangle(x, y, size, size, 0x000033, 0.8)
            .setOrigin(0, 0)
            .setStrokeStyle(2, 0x00ffff);
        
        // Mini-map label
        this.miniMapLabel = this.scene.add.text(x + size/2, y - 15, 'RADAR', {
            fontSize: '10px',
            fontFamily: 'Courier, monospace',
            color: '#00ffff'
        }).setOrigin(0.5, 0);
        ...
        // Player dot
        this.miniMapPlayer = this.scene.add.circle(x + size/2, y + size/2, 3, 0x00ff00);
        
        // Add to HUD
        this.hudContainer.add([this.miniMapBg, this.miniMapLabel, this.miniMapPlayer]);
    ...
    updateMathPowerMeter = (data) => {
        ...
        const powerPercent = data.mathPower / data.maxMathPower;
        const width = 196;
    ...
    removeEnemyFromMiniMap = (data) => {
        const enemyIndex = this.miniMapEnemies.findIndex(enemy => enemy.enemyId === data.id);
        if (enemyIndex !== -1) {
    ...
    createMathQuizOverlay(data) {
        ...
        // Quiz background
        this.mathQuizBg = this.scene.add.rectangle(
            centerX, centerY,
            400, 300,
            0x000033, 0.95
        ...
        // Quiz title
        this.mathQuizTitle = this.scene.add.text(
            centerX, centerY - 120,
            'MATH CHALLENGE',
            {
                fontSize: '24px',
        ...
        // Question text
        this.mathQuestionText = this.scene.add.text(
            centerX, centerY - 60,
            data.question || '2 + 2 = ?',
            {
                fontSize: '32px',
        ...
        // Timer bar
        this.mathTimerBg = this.scene.add.rectangle(
            centerX, centerY + 120,
            300, 10,
            0x333333
        ).setStrokeStyle(1, 0xffffff);
        
        this.mathTimerFill = this.scene.add.rectangle(
            centerX - 148, centerY + 120,
            296, 8,
            0x00ff00
        ).setOrigin(0, 0.5);
        ...
        // Instructions
        this.mathInstructions = this.scene.add.text(
            centerX, centerY + 140,
            'Click an answer or press 1-4',
            {
                fontSize: '12px',
                fontFamily: 'Courier, monospace',
                color: '#888888'
    ...
    createPauseOverlay() {
        ...
        this.pauseUI.background.fillRect(0, 0, this.scene.cameras.main.width, this.scene.cameras.main.height);
        ...
        // Pause title
        this.pauseUI.title = this.scene.add.text(centerX, centerY - 80, 'GAME PAUSED', {
            fontSize: '32px',
            fontFamily: config.fonts.primary,
            color: '#00ffff',
            stroke: '#000000',
        ...
        // Resume button
        this.pauseUI.resumeButton = this.createPauseButton(centerX, centerY - 20, 'RESUME', () => {
            this.resumeGame();
        });
        this.pauseUI.container.add(this.pauseUI.resumeButton.container);

        // Main menu button
        this.pauseUI.menuButton = this.createPauseButton(centerX, centerY + 40, 'MAIN MENU', () => {
            this.goToMainMenu();
        });
        this.pauseUI.container.add(this.pauseUI.menuButton.container);

        // Instructions
        const instructions = this.scene.add.text(centerX, centerY + 100, 'Press ESC or P to resume', {
            fontSize: '16px',
            fontFamily: config.fonts.primary,
            color: '#ffffff',
            alpha: 0.8
