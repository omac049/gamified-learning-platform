...
import { EducationalMenuScene } from "./scenes/EducationalMenuScene.js";
import { ShopScene } from "./scenes/ShopScene.js";
import { Week1MathScene } from "./scenes/Week1MathScene.js";
import { Week2ReadingScene } from "./scenes/Week2ReadingScene.js";
import { Week3ScienceScene } from "./scenes/Week3ScienceScene.js";
import { Week4HistoryScene } from "./scenes/Week4HistoryScene.js";
import { Week5CrossoverScene } from "./scenes/Week5CrossoverScene.js";
import { Week6FinalScene } from "./scenes/Week6FinalScene.js";
...
console.log("     Phaser v" + Phaser.VERSION + " (" + Phaser.RENDERER_TYPE + " | " + Phaser.AUDIO_TYPE + ")  " + "https://phaser.io");
...
game.events.once('ready', () => {
    ...
    if (game.input && game.input.on) {
        ...
        console.warn("Main: Game input not available yet, will try alternative approach");
    ...
    const checkAudioContext = () => {
        if (game.sound && game.sound.context) {
            if (game.sound.context.state === 'suspended' && !audioContextUnlocked) {
                console.log("Main: AudioContext still suspended, waiting for user interaction");
            }
        }
    };
...
window.addEventListener('error', (event) => {
    ...
    if (event.error && event.error.message && event.error.message.includes('hitAreaCallback is not a function')) {
        console.error('Main: HitAreaCallback error detected - this indicates an interactive object with corrupted hit area');
        console.error('Main: Stack trace:', event.error.stack);
        console.warn('Main: This error has been suppressed to prevent console flooding');
    ...
    if (event.error && event.error.message && (
        ...
        console.error('Main: Phaser input system error detected:', event.error.message);
...
window.addEventListener('unhandledrejection', (event) => {
    ...
    if (event.reason && event.reason.message && (
        ...
        console.warn('Main: Promise rejection suppressed to maintain game stability');
        event.preventDefault();
    }
});

// Enhanced Phaser input system protection
if (typeof Phaser !== 'undefined' && Phaser.Input) {
    // Override the problematic method with error protection
    const originalPointWithinHitArea = Phaser.Input.InputManager.prototype.pointWithinHitArea;
    if (originalPointWithinHitArea) {
        Phaser.Input.InputManager.prototype.pointWithinHitArea = function(gameObject, x, y) {
            try {
                ...
                if (gameObject.type === 'Graphics' && gameObject.input) {
                    ...
                    if (!gameObject.input.hitAreaCallback || 
                        typeof gameObject.input.hitAreaCallback !== 'function' ||
                        !gameObject.input.hitArea) {
                        
                        console.warn('Main: Graphics object with problematic input detected, using safe fallback');
                        
                        // Immediately clean up the Graphics object input
                        try {
                            gameObject.input = null;
                        } catch (cleanupError) {
                            console.warn('Main: Graphics cleanup failed:', cleanupError.message);
                        }
                ...
                if (gameObject.input && gameObject.input.hitAreaCallback && 
                    typeof gameObject.input.hitAreaCallback !== 'function') {
                    console.warn('Main: Detected corrupted hitAreaCallback on object:', {
                        ...
                        hitAreaType: gameObject.input.hitArea ? gameObject.input.hitArea.constructor.name : 'none',
                    ...
                    try {
                        ...
                        if (gameObject.type === 'Graphics') {
                            console.log('Main: Immediately cleaning up corrupted Graphics object');
                            ...
                            console.log('Main: Successfully cleaned up corrupted Graphics object');
                        } else {
                        ...
                        console.warn('Main: Could not clean up corrupted interactive object:', cleanupError.message);
                        ...
                        try {
                            ...
                            console.error('Main: Last resort cleanup failed:', lastResortError.message);
                        }
                    }
                ...
                if (gameObject.type === 'Graphics' && gameObject.input && gameObject.input.hitArea) {
                    // Ensure Graphics objects have valid hit areas
                    if (!gameObject.input.hitArea || typeof gameObject.input.hitArea !== 'object') {
                        console.warn('Main: Graphics object has invalid hit area, cleaning up');
                        try {
                            gameObject.input = null;
                        } catch (error) {
                            console.warn('Main: Error cleaning up Graphics hit area:', error.message);
                        }
                        return false;
                    }
                    ...
                    if (gameObject.input.hitArea instanceof Phaser.Geom.Rectangle) {
                        ...
                        if (hitArea.x > 100 || hitArea.y > 100) {
                            console.warn('Main: Graphics object appears to have absolute coordinate hit area, cleaning up');
                            try {
                                gameObject.input = null;
                            } catch (error) {
                                console.warn('Main: Error cleaning up Graphics absolute hit area:', error.message);
                            }
                            return false;
                        }
                    }
                ...
                console.warn('Main: Protected hitAreaCallback error:', {
                    ...
                    scene: gameObject && gameObject.scene ? gameObject.scene.scene.key : 'unknown',
                ...
                try {
                    ...
                    console.error('Main: Final cleanup failed:', finalCleanupError.message);
                }
    ...
    window.cleanupCorruptedInteractives = function() {
            ...
            game.scene.scenes.forEach(scene => {
                if (scene && scene.children && scene.children.list) {
                    scene.children.list.forEach(child => {
                        ...
                        if (child.input && child.input.hitAreaCallback && 
                            typeof child.input.hitAreaCallback !== 'function') {
                            console.warn('Main: Cleaning up corrupted interactive object in scene:', scene.scene.key);
                            try {
                        ...
                        if (child.type === 'Graphics' && child.input && child.input.hitArea) {
                            if (!child.input.hitArea || typeof child.input.hitArea !== 'object') {
                                console.warn('Main: Found Graphics object with invalid hit area, cleaning up');
                                try {
                                    child.removeInteractive();
                                    graphicsCleanedCount++;
                                    cleanedCount++;
                                    ...
                                    try {
                                        ...
                                        console.error('Main: Force cleanup failed:', forceError.message);
                                    }
                                }
                            }
                        }
                        ...
                        if (scene.scene.key === 'EducationalMenuScene' && child.type === 'Graphics' && child.input) {
                            // Proactively check for potential issues
                            if (child.input.hitAreaCallback && typeof child.input.hitAreaCallback !== 'function') {
                                console.warn('Main: Proactive cleanup of EducationalMenuScene Graphics object');
                                try {
                                    child.input = null;
                                    graphicsCleanedCount++;
                                    cleanedCount++;
                                } catch (error) {
                                    console.warn('Main: Proactive cleanup failed:', error.message);
                                }
                            }
                        }
                    });
        ...
        if (cleanedCount > 0) {
            console.log(`Main: Cleaned up ${cleanedCount} corrupted interactive objects (${graphicsCleanedCount} Graphics objects)`);
        }
        return cleanedCount;
    };
}
...
window.safeSetInteractive = function(gameObject, hitArea = null, options = {}) {
    ...
    try {
        ...
        if (gameObject.input && gameObject.input.hitAreaCallback && 
            typeof gameObject.input.hitAreaCallback !== 'function') {
            console.warn('Main: safeSetInteractive resulted in corrupted hitAreaCallback, removing interactive');
            gameObject.removeInteractive();
            return false;
        }
        ...
        try {
            ...
            console.warn('Main: Could not clean up after setInteractive error:', cleanupError.message);
        }
        return false;
    }
};
    ...
    if (game && game.scene) {
        ...
        if (game.scene.scenes && Array.isArray(game.scene.scenes)) {
            game.scene.scenes.forEach(scene => {
                if (scene && scene.input && scene.input.on) {
                    ...
                    console.log("Main: Added audio unlock handler to scene:", scene.scene ? scene.scene.key : 'unknown');
                }
            });
        } else {
            console.log("Main: Scene array not available yet, will add handlers when scenes are created");
        }
        ...
        if (game.scene.on) {
            game.scene.on('add', (scene) => {
                if (scene && scene.input && scene.input.on) {
                    ...
                    console.log("Main: Added audio unlock handler to new scene:", scene.scene ? scene.scene.key : 'unknown');
                }
            });
        }
        ...
        setInterval(() => {
            if (window.cleanupCorruptedInteractives) {
                ...
                if (cleanedCount > 0) {
                    console.log(`Main: Periodic cleanup removed ${cleanedCount} corrupted interactive objects`);
                }
            }
        ...
        setInterval(() => {
            if (game && game.scene && game.scene.getScene) {
                ...
                if (scene && scene.scene && scene.scene.isActive && scene.scene.isActive()) {
                    let cleanedCount = 0;
                    if (scene.children && scene.children.list) {
                        scene.children.list.forEach(child => {
                            if (child.type === 'Graphics' && child.input && 
                                ...
                                typeof child.input.hitAreaCallback !== 'function') {
                                console.warn('Main: EducationalMenuScene specific cleanup of Graphics object');
                                try {
                                    child.input = null;
                                    cleanedCount++;
                                } catch (error) {
                                    console.warn('Main: EducationalMenuScene cleanup failed:', error.message);
                                }
                            }
                        });
                    }
                    if (cleanedCount > 0) {
                        console.log(`Main: EducationalMenuScene specific cleanup removed ${cleanedCount} Graphics objects`);
                    }
                }
            }
        ...
        if (game.canvas) {
            game.canvas.addEventListener('mousemove', () => {
                ...
                if (game && game.scene && game.scene.getScene) {
                    const scene = game.scene.getScene('EducationalMenuScene');
                    if (scene && scene.scene && scene.scene.isActive && scene.scene.isActive() && scene.children && scene.children.list) {
                        scene.children.list.forEach(child => {
                            if (child.type === 'Graphics' && child.input && 
                                child.input.hitAreaCallback && 
                                typeof child.input.hitAreaCallback !== 'function') {
...
if (Phaser.GameObjects && Phaser.GameObjects.Graphics) {
    const originalSetInteractive = Phaser.GameObjects.Graphics.prototype.setInteractive;
    if (originalSetInteractive) {
        Phaser.GameObjects.Graphics.prototype.setInteractive = function(hitArea, callback, dropZone) {
            try {
                console.log('Main: Graphics setInteractive called, using safe auto-detection');
                ...
                if (hitArea && typeof hitArea === 'object' && !hitArea.x && !hitArea.y && !hitArea.width && !hitArea.height) {
                ...
                console.error('Main: Error in Graphics setInteractive override:', error);
                
                // Fallback: try the original method with just options
                try {
                    return originalSetInteractive.call(this, { useHandCursor: true });
                } catch (fallbackError) {
                    console.error('Main: Graphics setInteractive fallback failed:', fallbackError);
                    return this;
                }
            }
        };
