export class AchievementManager {
    constructor(progressTracker) {
        ...
        this.achievements = {
            ...
            mathWizard: {
                ...
                condition: { type: 'subject_mastery', subject: 'math', accuracy: 85, count: 25 }
            },
            readingChampion: {
                id: 'readingChampion',
                name: 'Reading Champion',
                description: 'Master reading comprehension with 85% accuracy over 25 questions',
                icon: '📚',
                category: 'subject',
                rarity: 'rare',
                points: 80,
                rewards: { coins: 150, experience: 100 },
                condition: { type: 'subject_mastery', subject: 'reading', accuracy: 85, count: 25 }
            },
            scienceExplorer: {
                id: 'scienceExplorer',
                name: 'Science Explorer',
                description: 'Discover science mastery with 85% accuracy over 25 questions',
                icon: '🔬',
                category: 'subject',
                rarity: 'rare',
                points: 80,
                rewards: { coins: 150, experience: 100 },
                condition: { type: 'subject_mastery', subject: 'science', accuracy: 85, count: 25 }
            },
            historyBuff: {
                id: 'historyBuff',
                name: 'History Buff',
                description: 'Become a history expert with 85% accuracy over 25 questions',
                icon: '🏛️',
                category: 'subject',
                rarity: 'rare',
                points: 80,
                rewards: { coins: 150, experience: 100 },
                condition: { type: 'subject_mastery', subject: 'history', accuracy: 85, count: 25 }
            },
            ...
            scholarlyWisdom: {
                ...
                condition: { type: 'character_ability_use', character: 'scholar', count: 10 }
            },
            mysticMastery: {
                id: 'mysticMastery',
                name: 'Mystic Mastery',
                ...
                points: 45,
                rewards: { coins: 80, experience: 60 },
                condition: { type: 'character_ability_use', character: 'mystic', count: 10 }
            },
            techMaster: {
                id: 'techMaster',
                name: 'Tech Master',
                ...
                points: 45,
                rewards: { coins: 80, experience: 60 },
                condition: { type: 'character_ability_use', character: 'techbot', count: 10 }
            },
    ...
    checkAchievementCondition(achievement, eventData) {
        ...
        switch (condition.type) {
            ...
            case 'session_accuracy':
                ...
                const accuracy = (this.sessionData.correctAnswers / this.sessionData.questionsAnswered) * 100;
            ...
            case 'single_question_speed':
                return eventData.responseTime && eventData.responseTime <= condition.time;
                
            case 'streak':
                return this.sessionData.currentStreak >= condition.value;
                
            case 'subject_mastery':
                return this.checkSubjectMastery(condition.subject, condition.accuracy, condition.count);
            ...
            case 'character_ability_use':
                ...
                if (!characterType || characterType.id !== condition.character) return false;
    ...
    checkSpeedChallenge(questionCount, timeLimit) {
        if (this.sessionData.speedChallengeProgress.length < questionCount) return false;
        
        const recentTimes = this.sessionData.speedChallengeProgress.slice(-questionCount);
    ...
    getRecentAnswers(count) {
        ...
        const correctAnswers = this.sessionData.correctAnswers;
    ...
    unlockAchievement(achievement) {
        ...
        if (achievementObj.rewards && achievementObj.rewards.experience) {
            this.progressTracker.awardCharacterExperience(achievementObj.rewards.experience);
        }
    ...
    getAchievementStats() {
        ...
        return {
            ...
            totalAchievements,
            completionPercent: Math.round((unlockedAchievements.length / totalAchievements) * 100),
            totalPoints,
            categoryStats
        };
    }
    ...
    getNextAchievements(limit = 3) {
        ...
        return achievementProgress
            .filter(achievement => !achievement.isUnlocked && achievement.progressPercent > 0)
