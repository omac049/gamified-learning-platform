...
export class ObjectPoolManager {
    ...
    spawn(poolName, ...args) {
        ...
        if (object) {
            ...
            if (object.initialize && typeof object.initialize === 'function') {
    ...
    logPoolStatus() {
        ...
        this.poolStats.forEach((stats, poolName) => {
            const hitRate = stats.reused + stats.created > 0 
                ? (stats.reused / (stats.reused + stats.created)) * 100 
                : 0;
            
            console.log(`${poolName}: Active: ${stats.currentActive}/${stats.maxSize}, ` +
