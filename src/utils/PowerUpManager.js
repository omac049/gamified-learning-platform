export class PowerUpManager {
    constructor(progressTracker) {
        ...
        this.powerUps = {
            ...
            elementalMastery: {
                ...
                rarity: 'epic',
                characterType: 'mystic',
                effect: { type: 'subject_mastery', subject: 'science', difficultyReduction: 1, rewardMultiplier: 2.0 }
            },
        ...
        Object.values(this.powerUps).forEach(powerUp => {
            ...
            if (!powerUp.characterType || powerUp.characterType === characterType?.id) {
                ...
                available.push({
                    ...powerUp,
                    canUse,
                    cooldownRemaining: canUse ? 0 : powerUp.cooldown - (Date.now() - lastUsed),
                    owned: this.progressTracker.progress.inventory.powerUps[powerUp.id] || 0
                });
            }
        });
    ...
    activatePowerUp(powerUpId) {
        ...
        const owned = this.progressTracker.progress.inventory.powerUps[powerUpId] || 0;
        if (owned <= 0) {
        ...
        if (Date.now() - lastUsed < powerUp.cooldown) {
            const remaining = Math.ceil((powerUp.cooldown - (Date.now() - lastUsed)) / 1000);
    ...
    useProtection() {
        ...
        for (const [powerUpId, activation] of this.activePowerUps) {
            if (activation.effect.type === 'protection' && this.isPowerUpActive(powerUpId)) {
                this.activePowerUps.delete(powerUpId);
                return true;
            }
        }
    ...
    calculateEnhancedRewards(baseRewards, subject) {
        ...
        let enhancedRewards = { ...baseRewards };
        
        // Apply coin multiplier
        enhancedRewards.coins = Math.floor(enhancedRewards.coins * effects.coinMultiplier);
        
        // Apply experience multiplier
        enhancedRewards.experience = Math.floor(enhancedRewards.experience * effects.experienceMultiplier);
        
        // Apply subject-specific bonuses
        if (effects.subjectBonuses[subject]) {
            enhancedRewards.coins = Math.floor(enhancedRewards.coins * effects.subjectBonuses[subject]);
            enhancedRewards.experience = Math.floor(enhancedRewards.experience * effects.subjectBonuses[subject]);
        }
        
        // Add power-up bonus indicator
        enhancedRewards.powerUpBonus = effects.coinMultiplier > 1.0 || effects.experienceMultiplier > 1.0;
    ...
    getPowerUpStatus() {
        const active = [];
        const available = this.getAvailablePowerUps().filter(p => p.canUse && p.owned > 0);
        
        this.activePowerUps.forEach((activation, powerUpId) => {
            if (this.isPowerUpActive(powerUpId)) {
                const remaining = activation.endTime > 0 
    ...
    getRandomPowerUpReward(rarity = 'common') {
        const availablePowerUps = Object.values(this.powerUps).filter(p => p.rarity === rarity);
    ...
    getRecommendations(sessionData) {
        ...
        if (characterType) {
            const characterPowerUps = Object.values(this.powerUps)
                .filter(p => p.characterType === characterType.id);
